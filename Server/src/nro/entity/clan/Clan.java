...
import nro.core.util.ErrorResolver;
...
//    }
    ...
    /**
     * Gỡ {@code clan_id} của <b>mọi</b> người chơi từng thuộc bang này.
     *
     * <p>Phải gọi cùng lúc với {@link #deleteDB(int)}. Xoá dòng trong bảng
     * {@code clan} mà để nguyên {@code player.clan_id} thì thành viên đang
     * offline lúc giải tán sẽ đăng nhập lại và trỏ vào một bang không còn tồn
     * tại; tệ hơn, nếu về sau có bang mới mang đúng id đó thì họ tự nhiên thành
     * thành viên của một bang lạ.</p>
     *
     * <p>Chạy thẳng bằng SQL chứ không lặp qua danh sách thành viên: danh sách
     * trong bộ nhớ chỉ có người đang trong game.</p>
     */
    public static void goClanIdCuaMoiNguoi(int idBang) {
        try (Connection con = ConnectDB.getConnection();) {
            ...
            ps.close();
            Logger.logln(Logger.PURPLE, "[BANG] Giai tan bang " + idBang
                    + ", go clan_id cua " + soDong + " nguoi choi");
        } catch (Exception e) {
            Logger.logException(Clan.class, e,
                    "Lỗi gỡ clan_id khi giải tán bang " + idBang);
        }
    }
