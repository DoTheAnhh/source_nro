...
import static nro.entity.player.Player.idOutfitHalloween;
import static nro.entity.player.Player.idOutfitMafuba;
...
public class Detu extends Player {
    ...
    /**
     * Tiền tố tên đệ theo bậc nâng cấp / thức tỉnh.
     *
     * <p><b>Đã bỏ hệ thống nâng cấp và thức tỉnh đệ tử</b>: giờ đệ chỉ khác nhau
     * ở LOẠI (thường, Mabư, Cell, Bill), nhận bằng trứng. Nên hàm này luôn trả
     * chuỗi rỗng.</p>
     *
     * <p>Vẫn giữ hàm và trường {@code thuctinh} chứ không xoá: gói gửi xuống
     * client và cột trong CSDL vẫn có chỗ cho chúng, xoá đi là phải sửa cả hai
     * đầu và làm hỏng dữ liệu đệ đang có.</p>
     */
...
//    }
 ...
 private void increasePoint() {
    ...
    /**
     * Id mũ dùng để tra bảng {@code head_avatar} ra ảnh avatar.
     *
     * <p>Tên hàm dễ gây hiểu nhầm: nó trả về <b>id mũ</b>, không phải id ảnh.
     * Client mới tra {@code head_avatar} ra ảnh — cùng đường với người chơi
     * (xem {@code Player.getHead()} và {@code DataGame.sendHeadAvatar()}).</p>
     *
     * <p>Trước đây hàm này bỏ qua cải trang, nên đệ tử mặc cải trang thì hình
     * trên bản đồ đúng ({@link #getHead()} có đọc ô cải trang) mà avatar trong
     * khung thông tin vẫn là mặt cũ. Thứ tự ưu tiên dưới đây <b>copy đúng</b>
     * {@link #getHead()}: loại đệ tử đặc biệt mà chưa biến hình thì thắng, còn
     * lại thì cải trang thắng.</p>
     */
    public short getAvatar() {
        // Chua bien hinh va la de tu loai dac biet -> mat cua loai do, giong
        // het nhanh "typeDeTu == ... && !isTransform" trong getHead().
        boolean uuTienLoai = !this.isTransform && this.typeDeTu != ConstDetu.NORMAL;
        if (!uuTienLoai) {
    ...
    /**
     * Id mũ của cải trang đệ tử đang mặc, hoặc {@code -1} nếu không mặc gì.
     *
     * <p>Ô 5 là ô cải trang. {@link #getHead()} đọc thẳng
    ...
    private short muTuCaiTrang() {
        ...
        nro.entity.item.Item ct = this.inventory.itemsBody.get(5);
        if (ct == null || !ct.isNotNullItem() || ct.template == null) {
            return -1;
        }
        int mu = ct.template.head;
        return mu != -1 ? (short) mu : -1;
    }
