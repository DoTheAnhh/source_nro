        ...
        if (cCongThuc == 1 && cManhThienSu == 1 && cDaNangCap <= 1 && cDaMayMan <= 1) {
            ...
            if (manhThienSu.quantity >= 999 && player.inventory.gold >= 200_000_000) {
                if (Util.isTrue(ratio, 100)) {
                    ...
                    //
                    // Ban cu viet nguoc: trung da may man thi boc 0..(10..15),
                    // tuc do ra YEU HON han binh thuong, trong khi hop thoai ghi
                    // "them X% ti le toi da cac chi so". Nguoi dung da may man
                    // cang xin cang bi lo.
                    int bonus = Util.isTrue(ratioDMM, 100)
                            ? Util.nextInt(25, 35)
