...
public class CombineUtil {
    ...
    //----------------------------------CHÂN MỆNH-------------------------------
    /**
     * Số Tinh thể / Ma quái cần để nâng chân mệnh từ cấp {@code star}.
     *
     * <p>Đọc từ bảng {@code chan_menh_cap} (tab "Chân mệnh" trên panel). Không
     * còn bảng switch viết cứng ở đây — một con số ở hai chỗ là sớm muộn lệch
     * nhau, và lệch kiểu này thì trong game trừ nguyên liệu khác với con số hiện
     * trên hộp thoại.</p>
     *
     * <p>Cấp chưa khai trả về {@code 0}: chỗ gọi hiểu là không đòi nguyên liệu,
     * chứ không phải đòi vô hạn.</p>
     */
    public static int getMaQuaiVaTinhTheNangcapChanmenh(int star) {
        nro.repository.dao.TrangSucDAO.Cap c =
                nro.repository.dao.TrangSucDAO.capChanMenhTheoCap(star);
        return c == null ? 0 : c.tinhThe;
    }


    /**
     * Tỉ lệ nâng chân mệnh từ cấp {@code star}, tính bằng <b>phần trăm thật</b>.
     *
     * <p>Đọc từ bảng {@code chan_menh_cap}. Chỗ gọi so với mẫu số 100.</p>
     *
     * <p>Cấp chưa khai trả về {@code 0} — không nâng được, thay vì âm thầm dùng
     * một tỉ lệ mặc định nào đó.</p>
     */
    public static float getTiLeNangcapChanmenh(int star) {
        nro.repository.dao.TrangSucDAO.Cap c =
                nro.repository.dao.TrangSucDAO.capChanMenhTheoCap(star);
        return c == null ? 0f : (float) c.tiLe;
    }
