        ...
        if (canOpenNpc(player)) {
            switch (this.mapId) {
                case 48:
                    ...
                    break;
                case 155:
                    // Muc "Doi vat pham" chi hien khi panel co gan cong thuc cho
                    // Whis: khong gan gi ma van hien la bam vao mo ra menu trong.
                    if (nro.repository.dao.CongThucDoiDAO.theoNpc(
                            nro.repository.dao.CongThucDoiDAO.NPC_WHIS_NGUC_TU)
                            .isEmpty()) {
    ...
    public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player)) {
            if (player.iDMark.getIndexMenu() == ConstNpc.WHIS_BILL) {
                ...
                if (select == 4) {
                    nro.service.combine.CongThucDoiService.gI().moDanhSachNpc(
                            player, this,
                            nro.repository.dao.CongThucDoiDAO.NPC_WHIS_NGUC_TU,
                            ConstNpc.WHIS_CONG_THUC_DOI);
                }
            } else if (player.iDMark.getIndexMenu() == ConstNpc.WHIS_CONG_THUC_DOI && this.mapId == 155) {
                nro.service.combine.CongThucDoiService.gI().chonCongThucNpc(
                        player, this,
                        nro.repository.dao.CongThucDoiDAO.NPC_WHIS_NGUC_TU,
                        select, ConstNpc.WHIS_DOI_XAC_NHAN);
            } else if (player.iDMark.getIndexMenu() == ConstNpc.WHIS_DOI_XAC_NHAN && this.mapId == 155) {
                if (select == 0) {
                    nro.service.combine.CongThucDoiService.gI()
                            .thucHienDangChon(player);
                } else {
