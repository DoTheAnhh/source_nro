            ...
            if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
                if (player.gender == ConstPlayer.XAYDA) {
                    if (!player.inventory.itemsDaBan.isEmpty()) {
                        this.createOtherMenu(player, ConstNpc.SHOP_APPULE_MUA_LAI,
                        "Ngươi cần trang bị gì cứ đến chỗ ta nhé", 
                        "Cửa\nhàng", "Mua lại\nvật phẩm\nđã bán\n[" + player.inventory.itemsDaBan.size() + "/" + BuyBack.MAX_COUNT_IN_BOX + "]", "Đóng");
                    } else {
                        this.createOtherMenu(player, ConstNpc.SHOP_APPULE_MUA_LAI_2,
                        "Ngươi cần trang bị gì cứ đến chỗ ta nhé", 
                        "Cửa Hàng",  "Đóng");
                    }
                } else {
                    ...
                    } else if (player.Detu != null && player.Detu.gender != 2) {
                        NpcService.gI().createTutorial(player, tempId, this.avartar, "Về hành tinh hạ đẳng của ngươi mà mua đồ cùi nhé. Tại đây ta chỉ bán đồ cho người Xayda thôi");
                    } else if (player.Detu != null && player.Detu.gender == 2) {
                        this.createOtherMenu(player, ConstNpc.SHOP_APPULE_2,
                            "Ngươi cần trang bị gì cứ đến chỗ ta nhé", 
        ...
        if (canOpenNpc(player)) {
            switch (player.iDMark.getIndexMenu()) {
                ...
                case ConstNpc.SHOP_APPULE_2:
                    ...
                    break;
                case ConstNpc.SHOP_APPULE_MUA_LAI:
                    switch (select) {
                        case 0:
                            ShopService.gI().opendShop(player, "APPULE", true);
                            break;
                    ...
                    break;
                case ConstNpc.SHOP_APPULE_MUA_LAI_2:
                    switch (select) {
                        case 0:
                            ShopService.gI().opendShop(player, "APPULE", true);
                            break;
