...
//import nro.service.map.worldmartialartstournament.WorldMartialArtsTournamentService;
...
//                }
//            } else {
//                super.confirmMenu(player, select);
//            }
//        }
//    }
    ...
    public void openBaseMenu(Player pl) {
        if (canOpenNpc(pl)) {
            if (this.map.mapId == 52) {
                ...
                this.createOtherMenu(pl, ConstNpc.BASE_MENU,
                        "Đấm vào máy đo sức mạnh đằng kia nhé.\n"
                                + "Vừa chạm tay vào máy là tôi bấm giờ 30 giây,\n"
                                + "hết 30 giây tôi chốt tổng sát thương lượt đó.\n"
                                + "Nghỉ tay rồi đấm lại là mở lượt mới.",
                        "Top\nĐòn mạnh\nnhất", "Top\nSát thương\n30 giây",
                        "Kỷ lục\ncủa tôi", "Đóng");
    ...
    public void confirmMenu(Player player, int select) {
        ...
        if (this.map.mapId == 42 || this.map.mapId == 43 || this.map.mapId == 44) {
            if (player.iDMark.isBaseMenu()) {
                switch (select) {
                    case 0:
                        TopService.showTopDonManhNhat(player);
                        break;
                    case 1:
                        TopService.showTopDame30s(player);
                        break;
                    case 2:
                        // Dùng menu (mã 32) chứ không dùng thoại NPC (mã 38):
                        // thoại NPC cắt chuỗi theo "\n" thành nhiều trang, mỗi
                        // trang một nút "Tiếp tục"; menu vẽ trọn mọi dòng trong
                        // một khung.
                        this.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "|7|KỶ LỤC MÁY ĐO SỨC MẠNH\n"
                                + "|1|Đòn mạnh nhất: "
                                + Util.format(player.donManhNhat) + " sát thương\n"
                                + "|2|Sát thương trong 30 giây: "
                                + Util.format(player.dame30s) + "\n"
                                + "|0|Đấm phát đầu là bắt đầu tính giờ\n"
                                + "|0|Hết 30 giây hoặc ngừng tay 3 giây thì chốt\n"
                                + "|0|Đấm lại là mở lượt mới",
                                "Đóng");
                        break;
                }
            }
                                ...
                                if (player.levelWoodChest != 12) {
                                    if (InventoryService.gI().finditemWoodChest(player)) {
                                        if (truThoiVang(player, SO_THOI_VANG_MOI_LAN)) {
                                            ...
                                            Service.gI().sendThongBao(player, "Bạn cần " + SO_THOI_VANG_MOI_LAN + " Thỏi vàng để thi đấu");
                                        }
                                    } else {
                                        Service.gI().sendThongBao(player, "Hãy mở rương báu vật trước");
                                    }
