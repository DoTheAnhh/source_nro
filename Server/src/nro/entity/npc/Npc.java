...
public abstract class Npc implements IAtionNpc {
    ...
    /**
     * Capsule kích hoạt tự chọn: người chơi chọn ô ({@code type} 0 áo … 4
     * rađa), bậc và set theo panel — xem {@link nro.service.fun.DoKichHoat}.
     * Kiểm đủ mọi thứ rồi mới trừ capsule.
     */
    public void giveCapsuleItem(Player pl, int type) {
        ...
        if (type < 0 || type >= nro.service.fun.DoKichHoat.SO_O) {
        ...
        int[] opSet = nro.service.fun.DoKichHoat.optionSet(pl.gender);
        if (opSet == null) {
            nro.service.fun.DoKichHoat.baoThieuSet(pl);
            return;
        }
        short idItem = nro.service.fun.DoKichHoat.mauDo(pl.gender, type, nro.service.fun.DoKichHoat.bocBac());
        if (idItem < 0) {
            Service.gI().sendThongBao(pl, "Không tìm thấy item phù hợp.");
            return;
        }
        Item it = nro.service.fun.DoKichHoat.tao(idItem, opSet, new int[]{213, 1});
