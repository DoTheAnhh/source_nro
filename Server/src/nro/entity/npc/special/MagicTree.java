package nro.entity.npc.special;
...
public class MagicTree {
    ...
    private static final int[] KET_NHANH = {2, 5, 8, 11, 14, 17, 20, 23, 26, 29};
    private static final int[] NANG_CAP_NHANH = {5, 10, 35, 128, 280, 512, 1208, 3264, 9900};
    ...
    public void openMenuTree() {
        ...
        try {
            ...
            if (!isUpgrade) {
                ...
                if (this.currPeas < this.getMaxPea()) {
                    msg.writer().writeUTF("Kết hạt nhanh " + MagicTree.KET_NHANH[level - 1] + " ngọc");
                ...
                msg.writer().writeUTF("Nâng cấp nhanh " + MagicTree.NANG_CAP_NHANH[level - 1] + " ngọc");
    ...
    public void fastRespawnPea() {
        int Money = MagicTree.KET_NHANH[level - 1];
        //giá kết hạt nhanh
    ...
    public void fastUpgradeMagicTree() {
        int Money = MagicTree.NANG_CAP_NHANH[level - 1];
        //giá nâng cấp nhanh
        if (player.inventory.gem >= Money) {
            if (this.level < MAX_LEVEL) {
                this.level++;
