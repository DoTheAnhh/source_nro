...
public class Zone {
        ...
        if (itemMap != null && itemMap.itemTemplate != null && !itemMap.isPickedUp) {
            ...
            if (playerId == player.id || itemMap.playerId == player.id || itemMap.playerId == -1) {
                ...
                if (InventoryService.gI().addItemBag(player, item)) {
                    // Nhiem vu danh hieu loai "nhat vat pham".
                    nro.service.badges.BadgesTaskService.tangTheoLoai(player,
                            nro.entity.badges.BadgesTaskTemplate.NHAT_VAT_PHAM,
                            item.template.id, 1);
            ...
            // Nhiem vu KOL da go — khong con dem vat pham nhat duoc nua.
    ...
    /**
     * Thả một vật phẩm xuống đất trong khu này.
     *
     * <p><b>Bỏ qua</b> nếu đang là lúc một boss trả thưởng và boss đó bật "chặn
     * đồ gốc" trong {@code boss_config}: khi đó đồ rơi viết cứng của nó được
     * thay hẳn bằng các dòng trong {@code boss_drop}. Chặn ở đây vì đây là chỗ
     * duy nhất mọi đường thả đồ đều đi qua — 118 lớp boss tự viết
     * {@code reward()} của riêng chúng.</p>
     */
    ...
    private void infoPlayer(Player plReceive, Player plInfo) {
        ...
        try {
            ...
            msg.writer().writeByte(0); // isTichXanh
...
//            }
    ...
    /**
     * Bày id + tên bản đồ + khu ra dòng chữ góc màn hình.
     *
     * <p>Ô chữ client vẽ sẵn chỉ in <code>0 - Khu: 0</code> — có id và khu nhưng
     * không có tên. Máy chủ đã gửi {@code mapName} trong chính gói {@code -24}
     * ngay trên, client không in ra; client là bản IL2CPP nên không sửa được.
     * Dòng này dùng ô chữ đếm ngược mà máy chủ điều khiển được.</p>
     *
     * <p>Gửi lại mỗi lần vào map nên không sợ hết giờ; ai đứng yên quá lâu thì
     * dòng tự tắt, đổi map một cái là hiện lại.</p>
     */
    private void hienTenBanDo(Player pl) {
        try {
            if (pl == null || this.map == null
                    || nro.repository.dao.ConfigDAO.num("hien_ten_map") == 0) {
                return;
            }
            nro.service.item.ItemTimeService.gI().sendTextTime(pl,
                    nro.entity.item.ItemTime.TEXT_MAP_HIEN_TAI,
                    this.map.mapName + " (id " + this.map.mapId + ") — Khu "
                            + this.zoneId,
                    32000);
        } catch (Exception ex) {
            // Chi la dong chu trang tri — hong thi bo qua, dung chan vao map.
            nro.core.log.Logger.logException(Zone.class, ex,
                    "Lỗi hiện tên bản đồ");
        }
    }
