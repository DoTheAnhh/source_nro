...
public class DataGame {
    ...
    /**
     * Phiên bản dữ liệu — <b>đọc từ bảng {@code panel_config}</b>, không còn viết
     * cứng ở đây.
     *
     * <p>Client giữ gói dữ liệu đã tải trong bộ nhớ đệm và chỉ tải lại khi con số
     * này đổi. Trước đây phải sửa mã nguồn rồi biên dịch lại mới tăng được, nên
     * thêm cải trang mới rất hay bị quên — người chơi mặc vào không thấy gì.
     * Nay tăng thẳng trên panel, tab "Cấu Hình → Quy ước".</p>
     *
     * <p>Các trường tĩnh dưới đây chỉ còn là giá trị dự phòng khi chưa đọc được
     * CSDL.</p>
     */
    ...
    private static byte vs(String khoa, byte duPhong) {
        try {
            return (byte) nro.repository.dao.ConfigDAO.num(khoa);
        } catch (Exception ex) {
            return duPhong;
        }
    }
    ...
    /**
     * Phiên bản bảng vật phẩm <b>đang có hiệu lực</b>.
     *
     * <h3>Vì sao phải đi qua một hàm chứ không đọc trực tiếp {@link #vsItem}</h3>
     *
     * <p>Client so <i>phiên bản server quảng bá</i> với phiên bản nó đang cất.
     * Khác nhau thì nó xin tải lại, rồi cất <b>con số đóng dấu trong chính gói dữ
     * liệu vừa nhận</b>.</p>
     *
     * <p>Nên hai chỗ đó buộc phải là một con số. Nếu {@code sendVersionGame} quảng
     * bá số lấy từ cấu hình mà {@code ItemData} lại đóng dấu hằng số, client sẽ
     * xin tải, cất số cũ, lần sau so lại vẫn khác, xin tải nữa — <b>lặp vô tận và
     * không bao giờ vào được game</b>. Không có lỗi nào được ném ra, không dòng
     * log nào: hai bên chỉ ngồi hỏi đáp mãi một câu.</p>
     */
    public static byte vsItemHienTai() {
        return vs(nro.repository.dao.ConfigDAO.VS_ITEM, vsItem);
    }

    /** Phiên bản bản đồ đang có hiệu lực. Xem {@link #vsItemHienTai()}. */
    public static byte vsMapHienTai() {
        return vs(nro.repository.dao.ConfigDAO.VS_MAP, vsMap);
    }

    /** Phiên bản kỹ năng đang có hiệu lực. Xem {@link #vsItemHienTai()}. */
    public static byte vsSkillHienTai() {
        return vs(nro.repository.dao.ConfigDAO.VS_SKILL, vsSkill);
    }

    /** Phiên bản gói dữ liệu đang có hiệu lực. Xem {@link #vsItemHienTai()}. */
    public static byte vsDataHienTai() {
        return vs(nro.repository.dao.ConfigDAO.VS_DATA, vsData);
    }
    ...
    public static void sendVersionGame(MySession session) {
        ...
        try {
            ...
            msg.writer().writeByte(vsDataHienTai());
            msg.writer().writeByte(vsMapHienTai());
            msg.writer().writeByte(vsSkillHienTai());
            msg.writer().writeByte(vsItemHienTai());
