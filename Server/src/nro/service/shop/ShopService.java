...
import nro.service.badges.BadgesService;
...
public class ShopService {
    ...
    /**
     * Bán theo <b>Thỏi vàng</b>.
     *
     * <p>Khác bốn loại trên: vàng/ngọc/hồng ngọc là các cột số trong
     * {@code inventory}, còn thỏi vàng là <b>một vật phẩm nằm trong hành trang</b>
     * (id {@value #ID_THOI_VANG}). Nên trừ tiền ở đây là trừ số lượng của chính
...
//                }
                ...
                for (TabShop tab : shop.tabShops) {
                    ...
                    for (ItemShop itemShop : tab.itemShops) {
                        ...
                        for (ItemOption option : itemShop.options) {
                            ...
                            // Tran cua khoang ngau nhien, -1 la tri co dinh.
                            // Client doc them so nay de hien "5 ~ 10".
        ...
        if (shop != null) {
            ...
            try {
                ...
                for (TabShop tab : shop.tabShops) {
                    ...
                    for (ItemShop itemShop : tab.itemShops) {
                        msg.writer().writeShort(itemShop.temp.id);
                        msg.writer().writeShort(itemShop.id);
                        msg.writer().writeInt(0); // gold price
                        msg.writer().writeInt(itemShop.cost);
                        msg.writer().writeByte(itemShop.typeSell);
                        msg.writer().writeInt(1); // quantity
    ...
    private boolean subMoneyByItemShop(Player player, ItemShop is) {
        ...
        // Nhiệm vụ danh hiệu loại "tiêu vàng" / "tiêu ngọc" — mỗi đơn vị tiền
        // tính một điểm, nên số lần cần đạt chính là số tiền phải tiêu.
        if (gold > 0) {
            nro.service.badges.BadgesTaskService.tangTheoLoai(player,
                    nro.entity.badges.BadgesTaskTemplate.TIEU_VANG, -1, gold);
        }
        if (gem > 0 || ruby > 0) {
            nro.service.badges.BadgesTaskService.tangTheoLoai(player,
                    nro.entity.badges.BadgesTaskTemplate.TIEU_NGOC, -1, gem + ruby);
        }
        return true;
    }

    /**
     * Trừ Thỏi vàng trong hành trang.
     *
     * <p>Kiểm tra <b>trước khi trừ</b> và trả {@code false} nếu không đủ — người
    ...
    public void buyItem(Player player, int itemTempId) {
        ...
        // dùng chung cho hầu hết cửa hàng.
        nro.service.badges.BadgesTaskService.tangTheoLoai(player,
   ...
   public void sellItem(Player pl, int where, int index) {
    ...
    /**
     * Chọn danh hiệu ở tab "Sở hữu" của NPC Santa.
     *
     * <h3>Bấm lại danh hiệu đang đeo = TẮT</h3>
     *
     * <p>Bản cũ từ chối với câu "Danh hiệu đang được sữ dụng, hãy chọn danh
     * hiệu khác" — nghĩa là <b>đã đeo thì không gỡ ra được nữa</b>, chỉ đổi
     * sang cái khác. Người chơi không có cách nào không đeo danh hiệu nào.</p>
     *
     * <p>Nay bấm lại chính danh hiệu đang đeo thì tắt nó. Không phải thêm mục
     * "không dùng danh hiệu" vào cửa hàng — cửa hàng dựng từ bảng
     * {@code item_shop} nên mỗi mục phải là một vật phẩm có thật.</p>
     */
    private void changeDanhHieu(Player pl, ItemShop is) {
        if (pl.lastTimeChangeBadges - System.currentTimeMillis() > 0) {
            Service.gI().sendThongBao(pl, "Vui lòng đợi "
                    + (pl.lastTimeChangeBadges - System.currentTimeMillis()) / 1000
                    + " giây nữa");
            return;
        }
        int idEffect = BagesTemplate.fineIdEffectbyIdItem(is.temp.id);
        if (pl.badges.idBadges == idEffect) {
            BadgesService.tatHet(pl);
            pl.badges.idBadges = -1;
            Service.gI().point(pl);
            Service.gI().sendThongBao(pl, "Đã bỏ danh hiệu " + is.temp.name
                    + ". Bấm lại để đeo.");
            pl.lastTimeChangeBadges = System.currentTimeMillis() + 30000;
            return;
        }
        if (BadgesService.timTheoId(pl, idEffect) == null) {
