package nro.service.npc;

import nro.entity.npc.Npc;
import nro.entity.boss.legacy.nguhanhson.GiaiPhongAnNgoKhong;
...
import nro.service.boss.ElecManager;
import nro.service.boss.GasManager;
import nro.service.boss.MackiManager;
import nro.service.boss.OilManager;
...
import nro.service.fun.ItemUseHandler;
...
import static nro.gameplay.dragon.SummonDragon.SHENRON_1_STAR_WISHES_1;
import static nro.gameplay.dragon.SummonDragon.SHENRON_1_STAR_WISHES_2;
import static nro.gameplay.dragon.SummonDragon.SHENRON_SAY;
...
//                    return gapthu(mapId, status, cx, cy, tempId, avatar);
                ...
                default:
                    return npcTheoCauHinh(mapId, status, cx, cy, tempId, avatar);
            }
        } catch (Exception e) {
        ...
        new Npc(-1, -1, -1, -1, ConstNpc.CON_MEO, 351) {
            ...
            public void confirmMenu(Player player, int select) {
                switch (player.iDMark.getIndexMenu()) {
                    case ConstNpc.IGNORE_MENU:
                        break;
                    // Menu "Ban do" cua client — hai tang, xem MapNhanhService.
                    case ConstNpc.BAN_DO_NHOM:
                        nro.service.MapNhanhService.gI().chonNhom(player, select);
                        break;
                    case ConstNpc.BAN_DO_DIEM:
                        nro.service.MapNhanhService.gI().chonDiem(player, select);
                        break;
...
//                        }
    ...
    /**
     * NPC <b>không có lớp xử lý riêng</b> — đọc cửa hàng từ bảng {@code shop}.
     *
     * <p>Trước đây nhánh {@code default} trả về một NPC có {@code confirmMenu}
     * rỗng, nên NPC thêm từ panel chỉ nói "Ta có thể giúp gì cho ngươi?" rồi
     * bấm gì cũng không ra gì — kể cả khi đã gán cửa hàng cho nó.</p>
     *
     * <p>Giờ: có cửa hàng thì bày nút "Cửa hàng"; không có thì giữ nguyên hành
     * vi cũ. Cửa hàng tra theo {@code npcId} trong {@link Manager#SHOPS}, tức
     * đúng chỗ tab "Quản Lý Bản Đồ" của panel đang ghi.</p>
     */
    private static Npc npcTheoCauHinh(int mapId, int status, int cx, int cy,
                                      int tempId, int avatar) {
        return new Npc(mapId, status, cx, cy, tempId, avatar) {
            ...
            public void openBaseMenu(Player player) {
                ...
                nro.entity.shop.Shop shop = cuaHangCua(tempId);
                if (shop == null) {
                    super.openBaseMenu(player);
                    return;
                }
                // Cau chao doc tu npc_template.loi_chao — sua duoc trong panel
                // (Quan Ly Ban Do -> Sua mau NPC). De trong thi dung cau mac dinh.
                this.createOtherMenu(player, ConstNpc.NPC_PANEL_CUA_HANG,
                        nro.repository.dao.MapShopDAO.loiChao(tempId),
                        "Cửa hàng", "Đóng");
            }
            ...
            public void confirmMenu(Player player, int select) {
                ...
                if (player.iDMark.getIndexMenu() != ConstNpc.NPC_PANEL_CUA_HANG
                        || select != 0) {
                    return;
                }
                nro.entity.shop.Shop shop = cuaHangCua(tempId);
                if (shop == null || shop.tagName == null) {
                    // Cua hang bi xoa giua chung -> khong im lang, noi ro.
                    nro.service.Service.gI().sendThongBao(player,
                            "Cửa hàng này hiện không mở.");
                    return;
                }
                nro.service.shop.ShopService.gI().opendShop(player, shop.tagName, false);
            }
        };
    }

    /**
     * Cửa hàng gắn với một mẫu NPC, hoặc {@code null} nếu không có.
     *
     * <p>Chỉ nhận cửa hàng <b>có thẻ hàng</b>: một cửa hàng rỗng mở ra là một ô
     * trống, khó hiểu hơn là không có nút nào.</p>
     */
    private static nro.entity.shop.Shop cuaHangCua(int tempId) {
        try {
            for (nro.entity.shop.Shop s : Manager.SHOPS) {
                if (s != null && s.npcId == tempId
                        && s.tabShops != null && !s.tabShops.isEmpty()) {
                    return s;
                }
            }
        } catch (Exception ex) {
            Logger.logException(NpcFactory.class, ex, "Lỗi tra cửa hàng NPC " + tempId);
        }
        return null;
    }
}
