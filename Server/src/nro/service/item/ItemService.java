...
public class ItemService {
    ...
    /**
     * Dựng vật phẩm từ một món bày bán.
     *
     * <p>Chỉ số nào có khoảng ngẫu nhiên ({@code paramMax > param}) thì bốc trị
     * ngay lúc này — mỗi người mua một con số riêng. Cửa hàng vẫn hiện cả khoảng
     * "5 ~ 10" vì nó đọc thẳng {@code ItemShop.options}, còn món trong hành trang
     * đã chốt nên {@code paramMax} trả về -1.</p>
     */
    public Item createItemFromItemShop(ItemShop itemShop) {
        ...
        for (ItemOption io : itemShop.options) {
            ItemOption moi = new ItemOption(io);
            if (io.paramMax > io.param) {
                moi.param = nro.core.util.Util.nextInt(io.param, io.paramMax);
            }
            moi.paramMax = -1;
            item.itemOptions.add(moi);
            removeAndAddOptionTemplate(item.itemOptions, moi.optionTemplate.id);
            removeAndAddOptionTemplateHSD(item.itemOptions, moi.optionTemplate.id);
            removeOptionTemplate(item.itemOptions, moi.optionTemplate.id);
        }
        return item;
    }
...
//                return 143; // Set Dame -> +150% KI
    ...
    /**
     * Chọn một món <b>đồ vải thô</b> (bộ khởi đầu) để làm đồ set kích hoạt.
     *
     * <p>Mỗi hàng trong {@link #randTempItemKichHoat} là một ô trang bị, và
     * <b>ba cặp đầu</b> của hàng là bộ cấp thấp nhất của ba hệ — Trái Đất "vải
     * 3 lỗ / vải đen / nhựa", Namếc "sợi len", Xayda "vải thô"; người chơi quen
     * gọi chung cả nhóm là đồ vải thô. Các cặp sau là những cấp cao hơn.</p>
     *
     * <p>Hàm này lấy đúng ba cặp đầu mỗi hàng, cộng rađa cấp 1.</p>
     */
    public int randTempItemKichHoatVaiTho(int gender) {
        int[][][] items = {
            {{0, 0}, {1, 1}, {2, 2}},        // áo
            {{0, 6}, {1, 7}, {2, 8}},        // quần
            {{0, 21}, {1, 22}, {2, 23}},     // găng
        ...
        int o = Util.nextInt(items.length);
        List<Integer> chon = new ArrayList<>();
        for (int[] cap : items[o]) {
            if (cap[0] == gender || cap[0] == 3) {
                chon.add(cap[1]);
            }
        }
        // Hang nao khong co mon cho he nay -> quay ve luat cu thay vi tra 0,
        // vi 0 la mot mon co that (Ao vai 3 lo) va se roi sai he.
        return chon.isEmpty() ? randTempItemKichHoat(gender)
                : chon.get(Util.nextInt(chon.size()));
    }
    ...
    /**
     * Gắn option set kích hoạt lên một món đồ.
     *
     * <p><b>Một chỗ duy nhất</b> quyết định món đồ set kích hoạt mang option của
     * set nào — mọi đường phát đồ (quái rơi, NPC, mở hộp) đều đi qua đây, nên
     * không có đường nào lọt ra phát nhầm set cũ.</p>
     *
     * <p>Khi quy ước {@code skh_chi_tu_panel} bật: chỉ lấy option của các set
     * đang định nghĩa trong bảng {@code set_kich_hoat}. <b>Chưa định nghĩa set
     * nào thì không gắn gì cả</b> — thà ra món không có set còn hơn ra món thuộc
     * một set đã ngừng có tác dụng.</p>
     *
     * @param nhieu {@code true} thì gắn bốn option (bộ "mới"), không thì hai
     */
    public void themOptionSetKichHoat(List<ItemOption> ops, int gender, boolean nhieu) {
        int[] ids = nro.repository.dao.SetBonusDAO.kemMoTa(
                nro.repository.dao.SetBonusDAO.optionSetMoi(nhieu ? 4 : 2, gender));
        if (ids == null) {
            // Chua co set nao tren panel -> khong gan option set, chi khoa.
            ops.add(new ItemOption(30, 0));
            return;
        }
        for (int id : ids) {
            ops.add(new ItemOption(id, 0));
        }
        ops.add(new ItemOption(30, 0));
    }

    /**
     * Hai option set kích hoạt cho một món đồ.
     *
     * <p>Khi {@code skh_chi_tu_panel} bật, <b>ưu tiên</b> option của các set đang
     * định nghĩa trên panel. Sửa ngay trong hàm này chứ không sửa từng chỗ gọi:
     * có mười chỗ khác nhau phát đồ set kích hoạt (quái rơi, NPC, mở hộp), bỏ sót
     * một chỗ là chỗ đó vẫn phát set cũ.</p>
     *
     * <p>Chưa định nghĩa set nào thì rơi về danh sách viết cứng — nhưng lúc đó
     * option cũ cũng <b>không tính thành set nào</b> nữa, vì phần đếm set viết
     * cứng đã tắt.</p>
     */
    public int[] randOptionItemKichHoat(int gender) {
        int[] moi = nro.repository.dao.SetBonusDAO.optionSetMoi(2, gender);
        // Chua dinh nghia set nao tren panel -> KHONG gan option set nao. Roi ve
        // danh sach viet cung la phat ra set da bo — dung thu vua xoa.
        return moi != null ? nro.repository.dao.SetBonusDAO.kemMoTa(moi) : new int[0];
    }


    /** Bốn option set kích hoạt. Cùng luật với {@link #randOptionItemKichHoat}. */
    public int[] randOptionItemKichHoatNew(int gender) {
        int[] moi = nro.repository.dao.SetBonusDAO.optionSetMoi(4, gender);
        return moi != null ? nro.repository.dao.SetBonusDAO.kemMoTa(moi) : new int[0];
    }
