...
public class ItemUseHandler {
    ...
    // Ghi lai so luong TRUOC khi cho vao tui.
    //
    // addItemBag() gop vat pham vao o cung loai roi dat quantity = 0 (nghia la
    // "da phat het roi"). Thong bao chay sau 2 giay nen doc ra 0 — nguoi choi
    // thay "Ban nhan duoc Nguyet An x0" du da nhan du.
    final int soLuongNhan = itemReceived.quantity;
    ...
    if (InventoryService.gI().getCountEmptyBag(pl) > 0) {
        ...
        new Thread(() -> {
            Functions.sleep(2000);
            Service.gI().sendThongBao(pl, "Bạn nhận được "
                    + itemReceived.template.name + " x" + soLuongNhan);
        }).start();
    } else {
        Service.gI().sendThongBao(pl, "Hàng trang đã đầy, cần một ô trống trong hành trang");
    }
