...
public class TransactionService implements Runnable {
    ...
    /**
     * Gian cach giua hai lan giao dich, tinh bang mili giay.
     *
     * <p>Truoc day la hang so cung 30000. Nay doc tu panel
     * (khoa {@code giao_dich_cho_giay}) nen doi la an ngay, khong phai build lai.</p>
     */
    private static int delayTrade() {
        return nro.repository.dao.ConfigDAO.giayChoGiaoDich() * 1000;
    }
            ...
            switch (action) {
                ...
                case ACCEPT_TRADE:
                    ...
                    if (plMap != null && plMap.isPlandBot()) {
                        ...
                        if (trade == null) {
                            if (action == SEND_INVITE_TRADE) {
                                if (Util.canDoWithTime(pl.iDMark.getLastTimeTrade(), delayTrade())
                                        && Util.canDoWithTime(plMap.iDMark.getLastTimeTrade(), delayTrade())) {
