...
import nro.entity.boss.legacy.nguhanhson.NguuMaVuong;
...
import static nro.entity.inventory.Inventory.LIMIT_GOLD;
...
public class UseItem {
                        ...
                        if (index != -1) {
                            ...
                            if (item.isNotNullItem()) {
                                if (item.template.type == 7) {
                                    // Sách kỹ năng: học thẳng, bỏ hộp "Bạn chắc
                                    // chắn học ...?".
                                    //
                                    // Hộp đó chẳng bảo vệ được gì: bấm "Có" xong
                                    // client gửi lại ACCEPT_USE_ITEM, mà nhánh ấy
                                    // gọi đúng useItem này chứ không kiểm tra thêm
                                    // điều kiện nào. Học nhầm sách cũng không mất
                                    // gì ngoài chính cuốn sách.
                                    //
                                    // Rương báu vật (570) và vệ tinh (type 22) vẫn
                                    // giữ hộp xác nhận — hai thứ đó dùng là mất.
    ...
    private void useItem(Player pl, Item item, int indexBag) {
        ...
        if (item != null && item.isNotNullItem()) {
            // Nhiệm vụ danh hiệu loại "dùng vật phẩm". Đếm ở đây vì đây là cửa
            // duy nhất mọi lần bấm dùng đồ đi qua.
                ...
                switch (item.template.type) {
                    ...
                    default:
                        switch (item.template.id) {
                            ...
                            // 2451 = The doi ten.
                            //
                            // Nhanh nay truoc ghi 2006, nhung 2006 trong
                            // item_template la "Chan thien tu ve binh" —
                            // mot mon do chan, khong phai the doi ten. Nen
                            // duong doi ten bang vat pham chua bao gio chay.
                            case 2451:
                            ...
                            case 2290:
                                ...
                                break;
                            default:
                                // Goi cua su kien: noi dung khai trong panel
                                // (bang su_kien_hop), khong go cung o day nen
                                // them mot cai goi moi khong phai sua ma.
                                nro.service.event.SuKienService.moHop(pl, item);
                                break;
                        }
                        break;
                }
    ...
    private void learnSkill(Player pl, Item item) {
        ...
        try {
            if (item.template.gender == pl.gender || item.template.gender == 3) {
                ...
                // curSkill == null khi nguoi choi CHUA CO dong nao cho ky nang
                // do — dung voi moi ky nang vua them vao may chu. Truoc day
                // doc thang curSkill.point la NPE, bi try/catch nuot, bam sach
                // khong co gi xay ra va cung khong bao gi.
                int diemHienTai = curSkill == null ? 0 : curSkill.point;
                // maxPoint PHAI lay tu template THAT trong Manager. Ky nang
                // chua hoc duoc dung boi createSkillLevel0(), ham do dung mot
                // SkillTemplate RONG chi co id — maxPoint = 0. Doc maxPoint tu
                // do thi 0 >= 0 la "da dat toi da", va MOI quyen sach cap 1
                // deu bao full cap.
                nro.entity.template.SkillTemplate mauThat =
                        SkillUtil.findSkillTemplate(
                                SkillUtil.getTempSkillSkillByItemID(
                                        item.template.id));
                int diemToiDa = mauThat != null && mauThat.maxPoint > 0
                        ? mauThat.maxPoint : 7;
                if (diemHienTai >= diemToiDa) {
                    ...
                    if (diemHienTai == 0) {
                        ...
                        if (diemHienTai + 1 == level) {
                            ...
                            Service.gI().sendThongBao(pl, "Vui lòng học "
                                    + curSkill.template.name + " cấp "
                                    + (diemHienTai + 1) + " trước!");
                        }
                    }
                    InventoryService.gI().sendItemBag(pl);
    ...
    private void openCapsuleUI(Player pl) {
        ...
        Logger.logln(Logger.PURPLE, "[CAPSULE] " + pl.name
                + " mo UI capsule, dat typeChangeMap="
                + ConstMap.CHANGE_CAPSULE);
    ...
    public void choseMapCapsule(Player pl, int index) {
        ...
        int zoneId = -1;
        if (pl.mapCapsule == null || index > pl.mapCapsule.size() - 1
                || index < 0) {
            // Danh sach chua duoc dung, hoac chi so ngoai khoang. Ghi lai de doc
            // duoc tu log: hai truong hop nay nhin tu trong game giong het nhau.
            Logger.logln(Logger.PURPLE, "[CAPSULE] " + pl.name
                    + " chi so sai: " + index + ", danh sach "
                    + (pl.mapCapsule == null ? "null"
                            : String.valueOf(pl.mapCapsule.size())));
            Service.gI().sendThongBao(pl, "Không thể thực hiện");
            Service.gI().hideWaitDialog(pl);
            return;
        }
        ...
        // Bon dieu kien chan, moi dieu kien noi RO ly do.
        //
        // Ban truoc gop ca bon vao mot cau "Hien tai khong the vao duoc khu!" va
        // — quan trong hon — thoat ma KHONG tat hop "Xin cho". Client mo hop do
        // ngay khi gui lua chon, nen nguoi choi ngoi mai o "Xin cho" va khong biet
        // vi sao khong bay: nhin nhu chon map xong khong co gi xay ra.
        String vuong = null;
        if (zoneChose.getNumOfPlayers() > 25) {
            vuong = "Khu này đã quá đông, chọn bản đồ khác.";
        } else if (MapService.gI().isMapDoanhTrai(zoneChose.map.mapId)) {
            vuong = "Không đi capsule vào doanh trại được.";
        } else if (MapService.gI().isMapMaBu12H(zoneChose.map.mapId)) {
            vuong = "Không đi capsule vào map Ma Bư được.";
        } else if (MapService.gI().isMapBanDoKhoBau(zoneChose.map.mapId)) {
            vuong = "Không đi capsule vào bản đồ kho báu được.";
        }
        if (vuong != null) {
            Logger.logln(Logger.PURPLE, "[CAPSULE] " + pl.name + " bi chan: "
                    + vuong + " (map " + zoneChose.map.mapId + ")");
            Service.gI().sendThongBao(pl, vuong);
            Service.gI().hideWaitDialog(pl);
            return;
        }
        Logger.logln(Logger.PURPLE, "[CAPSULE] " + pl.name + " di tới map "
                + zoneChose.map.mapId + " (chi so " + index + ")");
