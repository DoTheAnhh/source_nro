...
import static nro.core.consts.ConstNpc.QUY_DOI_HN;
...
import nro.gameplay.giftcode.GiftCodeService;
        ...
        try {
            ...
            switch (player.iDMark.getTypeInput()) {
                ...
                case DISSOLUTION_CLAN: {
                    ...
                    if (xacNhan.equalsIgnoreCase("OK")) {
                        ...
                        if (clan != null && clan.isLeader(player)) {
                            ...
                            // Duyet BAN SAO cua danh sach thanh vien.
                            //
                            // Vong nay go member.clan, va changeMap co the doi ca
                            // membersInGame — sua danh sach dang duyet la
                            // ConcurrentModificationException, no bay ra giua chung
                            // va nhung nguoi con lai khong duoc go.
                            java.util.List<Player> dsTrongGame =
                                    new java.util.ArrayList<>(clan.membersInGame);
                            for (Player member : dsTrongGame) {
                                if (member == null || member == player) {
                                    continue;
                                }
                                // Chi keo ra khoi map bang nhung nguoi DANG o do.
                                //
                                // Ban truoc dat ca viec go bang vao trong dieu kien
                                // nay, nen ai dang dung o map khac thi giu nguyen
                                // member.clan tro vao mot bang da bi xoa — mot tham
                                // chieu treo, va trong game van thay minh con bang.
                                if (member.zone != null && member.zone.map != null
                                        && MapService.gI().isMapBangHoi(
                                                member.zone.map.mapId)) {
                                    ChangeMapService.gI().changeMap(member, 5, -1,
                                            Util.nextInt(1050, 1150), 408);
                                }
                                member.clan = null;
                                member.clanMember = null;
                                ClanService.gI().sendMyClan(member);
                                ClanService.gI().sendClanId(member);
                                Service.gI().sendThongBao(member,
                                        "Bang hội của bạn đã bị giải tán.");
                            }

                            if (player.zone != null && player.zone.map != null
                                    && MapService.gI().isMapBangHoi(
                                            player.zone.map.mapId)) {
                                ChangeMapService.gI().changeMap(player, 5, -1,
                                        Util.nextInt(1050, 1150), 408);
                            }

                            clan.deleteDB(idBang);

                            // Go clan_id cua nhung nguoi DANG OFFLINE.
                            //
                            // deleteDB chi xoa dong trong bang `clan`. Thanh vien
                            // offline van giu clan_id tro vao bang vua xoa, nen
                            // dang nhap lai la vao mot bang khong ton tai — va neu
                            // ve sau co bang moi trung id thi ho tu nhien thanh
                            // thanh vien cua bang la.
                            Clan.goClanIdCuaMoiNguoi(idBang);
                            ...
                            Service.gI().sendThongBao(player,
                                    "Bang hội đã giải tán thành công.");
                        }
                    }
                    break;
                }
                ...
                case SLLTV: {
                    ...
                    // Ban thoi nao cung duoc (khoa hay thuong). Vang ban ra giao dich
                    // duoc, nhung da co han muc vang giao dich moi ngay (GiaoDichVangDAO).
                    ...
                    if (!nro.gameplay.minigame.KhoVang.tru(player, sltv)) {
                ...
                case PASS_ADMIN: {
                    if (player.isFounder()) {
                        ...
                        // Mat khau doc tu panel_config chu khong viet cung nua:
                        // doi mat khau ma phai bien dich lai server thi khong ai
                        // doi duoc, va no nam nguyen trong ma nguon ai doc cung thay.
                        String matKhauCuaToi = nro.repository.dao.ConfigDAO.chuoi(
                                nro.repository.dao.ConfigDAO.MAT_KHAU_ADMIN);
                        if (matKhauNhap.equals(matKhauCuaToi)) {
                            ...
                            // Mo luon panel quan tri o may chay server.
                            if (nro.repository.dao.ConfigDAO.num(
                                    nro.repository.dao.ConfigDAO.MO_PANEL_KHI_DUNG_PASS) == 1) {
                                boolean daMo = nro.ui.ServerManagerUI.moLenTruoc();
                                Service.gI().sendThongBao(player, daMo
                                        ? "Đã mở panel quản trị trên máy chủ."
                                        : "Máy chủ đang chạy không giao diện, không có panel để mở.");
                            }
                        } else {
                ...
                case CHANGE_NAME_BY_ITEM: {
                    if (player != null) {
                        String tenMoi = chuanHoaTen(text[0]);
                        String loi = loiTenNhanVat(tenMoi);
                        if (loi != null) {
                            Service.gI().sendThongBao(player, loi);
                            createFormChangeNameByItem(player);
                        } else if (ConnectDB.executeQuery("select * from player where name = ?", tenMoi).next()) {
                            Service.gI().sendThongBao(player, "Tên nhân vật đã tồn tại");
                            createFormChangeNameByItem(player);
                        } else {
                            Item theDoiTen = InventoryService.gI().findItem(player.inventory.itemsBag, 2451);
                            if (theDoiTen == null) {
...
//---------------------------------------------------------------------------------------                    
                ...
                case DOI_THOI_VANG: {
                    ...
                    // Han muc toi thieu va ty le lay tu bang panel_config, admin sua
                    // tren panel la co hieu luc ngay, khong can khoi dong lai may chu.
                    long min = nro.repository.dao.ConfigDAO.num(
                            nro.repository.dao.ConfigDAO.DOI_TV_MIN);
                    if (vnd < min || vnd > 2_000_000_000) {
                        Service.gI().sendThongBao(player, "Số lượng không hợp lệ! (Phải từ "
                                + Util.format(min) + " đến 2.000.000.000)");
                        return;
                    }
                    ...
                    long ThoiVang1k = nro.repository.dao.ConfigDAO.num(
                            nro.repository.dao.ConfigDAO.DOI_TV_MOI_1K);
                    // Chia truoc roi moi nhan: (vnd/1000) lam tron xuong nen nguoi choi
                    // khong bao gio duoc nhieu hon so tien thuc su bi tru.
                    int soLuong = (int) ((vnd / 1000L) * ThoiVang1k);
                    if (soLuong <= 0) {
    ...
    /// Bo khoang trang thua o hai dau va gom nhieu dau cach lien nhau thanh mot.
    ///
    /// Nguoi choi hay go nham hai dau cach, hoac de mot dau cach o cuoi. Neu
    /// khong don thi hai nhan vat co the mang hai cai ten trong nhu het suc
    /// giong nhau, ma cau kiem tra trung ten lai khong bat duoc.
    ...
    /// Tra ve loi cua ten, hoac null neu ten dung.
    ///
    /// Cho phep chu cai (ke ca chu co dau tieng Viet), chu so va dau cach. Hoa
    /// thuong giu nguyen nhu nguoi choi go. Khong cho ky tu dieu khien hay dau
    /// cau, vi ten con di vao cau chat, bang xep hang va ten bang hoi.
    ///
    /// Do dai toi da 20 dung bang co cot name cua bang player: dai hon thi cau
    /// update se bi cat cut hoac bao loi.
    private static String loiTenNhanVat(String ten) {
        if (ten == null || ten.length() < 3) {
            return "Tên phải có ít nhất 3 ký tự";
        }
        if (ten.length() > 20) {
            return "Tên không được dài quá 20 ký tự";
        }
        for (int i = 0; i < ten.length(); i++) {
            char c = ten.charAt(i);
            if (c != ' ' && !Character.isLetterOrDigit(c)) {
                return "Tên chỉ được dùng chữ, số và dấu cách";
            }
        }
        return null;
    }
