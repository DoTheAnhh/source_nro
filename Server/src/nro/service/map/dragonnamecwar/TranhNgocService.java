package nro.service.map.dragonnamecwar;
...
public class TranhNgocService {
                ...
                for (Player pl : players) {
                    if (pl != null) {
                        if (pl.event.getNamekWarPoint() >= 7) {
                            ...
                            Service.gI().sendThongBao(pl, "Bạn đã thua và bị thu " + pointDiff + " Điểm chiến trường Namek");
                        }
            ...
            case ConstTranhNgocNamek.WIN:
                for (Player pl : players) {
                    if (pl != null) {
                        ...
                        InventoryService.gI().sendItemBag(pl);
                        Service.gI().sendThongBao(pl, "Bạn đã thắng và nhận " + point + " Điểm chiến trường Namek");
                        TranhNgoc.gI().removePlayersCadic(pl);
                        TranhNgoc.gI().removePlayersFide(pl);
                    }
                }
            ...
            case ConstTranhNgocNamek.DRAW:
                for (Player pl : players) {
                    if (pl != null) {
                        ...
                        InventoryService.gI().sendItemBag(pl);
                        Service.gI().sendThongBao(pl, "2 bên hòa nhau và nhận " + (point / 2) + " Điểm chiến trường Namek");
                        TranhNgoc.gI().removePlayersCadic(pl);
                        TranhNgoc.gI().removePlayersFide(pl);
                    }
                }
    ...
    public void pickBall(Player player, ItemMap item) {
        if (player.isHoldNamecBallTranhDoat || item.typeHaveBallTranhDoat == player.iDMark.getTranhNgoc()) {
             Service.gI().sendThongBao(player, "Ngọc rồng này đã được sử dụng rồi!");
            return;
        }
