...
public class FriendAndEnemyService {
    ...
    private void reloadFriend(Player player) {
        for (Friend f : player.friends) {
            Player pl;
            if ((pl = Client.gI().getPlayerByUser(f.id)) != null || (pl = Client.gI().getPlayerByName(f.name)) != null) {
                try {
    ...
    private void reloadEnemy(Player player) {
        for (Enemy e : player.enemies) {
            Player pl;
            if ((pl = Client.gI().getPlayerByUser(e.id)) != null || (pl = Client.gI().getPlayerByName(e.name)) != null) {
                try {
    ...
    private void openListFriend(Player player) {
        ...
        try {
            ...
            for (Friend f : player.friends) {
                ...
                msg.writer().writeBoolean(Client.gI().getPlayerByID((int) f.id) != null);
                msg.writer().writeUTF(Util.formatNumber(f.power, FormatStyle.VIETNAMESE));
            }
    ...
    private void openListEnemy(Player player) {
        ...
        try {
            ...
            for (Enemy e : player.enemies) {
                ...
                msg.writer().writeUTF(Util.formatNumber(e.power, FormatStyle.VIETNAMESE));
                msg.writer().writeBoolean(Client.gI().getPlayerByID((int) e.id) != null);
            }
            ...
            if (pl != null) {
                if (player.isFounder() || player.nPoint.teleport) {
                    if (!pl.itemTime.isUseAnDanh || player.isFounder()) {
                        if ((player.isFounder() || !pl.zone.isFullPlayer())
                                && !MapService.gI().isMapOffline(pl.zone.map.mapId)
                                && !MapService.gI().isMapBangHoi(pl.zone.map.mapId)
                            ...
                            ChangeMapService.gI().changeMapYardrat(
                                    ...
                                    pl.location.x + Util.nextInt(-5, 5),
                                    pl.location.y
                            );
                        } else {
                            Service.gI().sendThongBao(player, "Không thể thực hiện");
