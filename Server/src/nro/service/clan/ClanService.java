package nro.service.clan;

import nro.entity.clan.Clan;
import nro.entity.clan.ClanMember;
import nro.entity.clan.ClanMessage;
import java.util.List;
import static nro.entity.clan.Clan.DEPUTY;
import static nro.entity.clan.Clan.LEADER;
import static nro.entity.clan.Clan.MEMBER;
    ...
    public void clanDonate(Player plGive, Message msg) {
        ...
        if (clan != null) {
            try {
                ...
                if (cmg != null) {
                    // Không tự cho đậu cho chính mình.
                    //
                    // Đường cũ lấy đậu từ rương rồi trả lại vào hành trang của
                    // CHÍNH người gửi, tức một vòng chuyển đồ không mất gì mà vẫn
                    // cộng receiveDonate và vẫn tính thành tích "hỗ trợ đồng
                    // đội". Xin đậu là để người KHÁC cho.
                    if (cmg.playerId == plGive.id) {
                        Service.gI().sendThongBao(plGive,
                                "Bạn không thể tự cho đậu cho chính mình");
                        return;
                    }
