...
import nro.gameplay.giftcode.GiftCode;
...
public class InventoryService {
    ...
    public void addItemGiftCodeToPlayer(Player p, GiftCode giftcode, String code) throws Exception {
        ...
        // Tham so hoa: `code` den truc tiep tu input nguoi choi.
...
//                        textGift += "x1 Danh Hiệu Bất Bại\nx1 Danh Hiệu Đại Thần\nx1 Danh Hiệu Trùm Cuối\b";
                        ...
                        if (itemGiftTemplate != null) {
                            ...
                            if (itemGift.template.type == 12 || itemGift.template.type >= 0 && itemGift.template.type <= 5) {
                                if (itemGift.template.id == 457) {
                                    ...
                                    // optionCua() tra BAN SAO rieng cua tung vat
                                    // pham. Truoc day gan thang giftcode.option
                                    // nen moi mon phat ra dung chung MOT doi
                                    // tuong danh sach — sua mon nay la doi luon
                                    // mon kia va ca ma goc trong bo nho.
                                    itemGift.itemOptions = giftcode.optionCua(idItem);
                            ...
                            ConnectDB.executeUpdate("UPDATE `giftcode` SET `count_left` = ? WHERE `code` = ? LIMIT 1;", trucount, code);
                        }
                        break;
                }
            }
            ConnectDB.executeUpdate(
                    "INSERT INTO `giftcode_save` (`id`,`player_id`,`code_da_nhap`,`tgian_nhap`) VALUES (?, ?, ?, ?);",
                    p.getSession().userId, p.id, code, Util.toDateString(Date.from(Instant.now())));
    ...
    /**
     * Dồn ô trống về cuối và <b>xếp vật phẩm theo loại</b>.
     *
     * <h3>Cơ chế cũ và vì sao bỏ</h3>
     *
     * <p>Bản cũ tìm ô trống đầu tiên, tìm món <b>cuối cùng</b>, đổi chỗ hai cái
     * rồi lặp lại. Đó là "dồn ô", không phải sắp xếp — và nó gây đúng cái khó
     * chịu người dùng mô tả: dùng hết một món ở giữa túi thì <b>món cuối cùng
     * nhảy lên chỗ vừa trống</b>. Túi xáo trộn sau mỗi lần dùng đồ, không bao
     * giờ nhớ được món nào nằm đâu.</p>
     *
     * <h3>Cách mới</h3>
     *
     * <p>Gom hết món thật, xếp theo <b>loại</b> trước ({@code template.type}),
     * rồi tới <b>độ hiếm</b> ({@code template.level}, hiếm lên đầu), rồi tới
     * <b>id</b>. Đặt lại từ ô 0, phần còn lại là ô trống.</p>
     *
     * <p>Loại đứng trước độ hiếm vì túi được đọc theo <i>nhóm</i>: mũ nằm cạnh
     * mũ, áo cạnh áo, thuốc cạnh thuốc — mắt tìm theo hình dạng ô chứ không tìm
     * theo bậc hiếm. Bản trước xếp hiếm trước, nên năm cái mũ của cùng người
     * chơi nằm rải ở năm chỗ khác nhau trong túi. Trong cùng một loại thì mới
     * xếp hiếm lên đầu.</p>
     *
     * <p>Thứ tự vẫn <b>ổn định</b>: dùng hết một món thì mọi thứ chỉ dồn lên một
     * ô, không có món nào nhảy từ cuối túi lên.</p>
     *
     * <p>Giữ nguyên <b>đúng các đối tượng ô trống cũ</b> chứ không tạo mới:
     * chỗ khác trong mã có thể đang giữ tham chiếu tới chúng.</p>
     */
    public void sortItems(List<Item> list) {
        if (list == null || list.isEmpty()) {
            return;
        }
        List<Item> co = new java.util.ArrayList<>(list.size());
        List<Item> rong = new java.util.ArrayList<>(list.size());
        for (Item it : list) {
            if (it != null && it.isNotNullItem()) {
                co.add(it);
            } else {
        ...
        co.sort(java.util.Comparator
                .comparingInt(InventoryService::loaiCua)
                .thenComparingInt(InventoryService::hiemCua)
                .thenComparingInt(InventoryService::idCua));
        int i = 0;
        for (Item it : co) {
            list.set(i++, it);
        }
    ...
    /**
     * Bậc hiếm của một món, <b>đã đảo dấu</b> để hiếm nhất lên đầu.
     *
     * <p>{@code template.level} càng lớn càng hiếm, mà bộ so sánh xếp tăng dần —
     * nên đảo dấu ở đây thay vì gọi {@code reversed()}: {@code reversed()} sẽ đảo
     * cả hai khoá phía sau, làm loại và id cũng chạy ngược.</p>
     *
     * <p>Món hỏng mẫu đẩy xuống cuối thay vì làm sập sắp xếp.</p>
     */
    private static int hiemCua(Item it) {
        if (it == null || it.template == null) {
            return Integer.MAX_VALUE;
        }
        return -it.template.level;
    }

    /** Loại của một món; món hỏng mẫu thì đẩy xuống cuối thay vì làm sập sắp xếp. */
    private static int loaiCua(Item it) {
        return it == null || it.template == null ? Integer.MAX_VALUE : it.template.type;
    }

    private static int idCua(Item it) {
        return it == null || it.template == null ? Integer.MAX_VALUE : it.template.id;
    }
...
//                break;                
        ...
        // index van la -1 khi type cua vat pham khong co trong bang tren — vi du
        // Linh Thu (type 70..75) van duoc UseItem goi sang day nhung o slot cua
        // chung dang bi tat. Truoc day chi chan "index >= size" nen roi thang vao
        // itemsBody.get(-1) va nem IndexOutOfBounds ra tan onMessage.
        //
        // Nhanh else cung tung cast (Detu) player cho nguoi KHONG phai de tu ->
        // ClassCastException. Nguoi thuong thi gui thang cho chinh ho.
        if (index < 0 || index >= player.inventory.itemsBody.size()) {
            Service.gI().sendThongBao(
                    player.isDeTu ? ((Detu) player).master : player,
                    player.isDeTu ? "Đệ tử không thể sử dụng vật phẩm này !"
...
//                return true;
    ...
    public boolean addItemBag(Player player, Item item) {
        ...
        //
        // Ba lỗi của bản cũ, đều làm người chơi mất vật phẩm mà không được gì:
        //  1. Không chặn ở mức tối đa. Cửa hàng chặn mua khi đã 100 ô, nhưng
        //     vật phẩm lấy từ nguồn khác (rơi, quà, giao dịch) thì dùng được
        //     mãi — vượt Inventory.MAX_ITEMS_BAG là client không vẽ nổi.
        //  2. Không gửi lại hành trang cho client, nên ô mới KHÔNG hiện ra cho
        //     tới lần đăng nhập sau. Người chơi tưởng vật phẩm hỏng.
        //  3. Vật phẩm 988 chỉ IN THÔNG BÁO chứ không tăng gì — xem chú thích
        //     ở nhánh 988 bên dưới.
        switch (item.template.id) {
            case 1627:
            case 517: {
                // Trần lấy ĐÚNG theo luật cửa hàng đang chạy (ShopService):
                // vật phẩm 1627 mở tới 150 ô, vật phẩm 517 tới 100. Hằng số
                // Inventory.MAX_ITEMS_BAG ghi 100 nhưng cửa hàng vẫn bán 1627
                // tới 150 — theo cửa hàng để không đổi hành vi hiện có.
                int tran = item.template.id == 1627 ? 150 : Inventory.MAX_ITEMS_BAG;
                if (player.inventory.itemsBag.size() >= tran) {
                    Service.gI().sendThongBao(player,
                            "Hành trang đã đạt tối đa " + tran + " ô.");
                    return false;
                }
                ...
                Service.gI().sendThongBaoOK(player,
                        ...
                        + player.inventory.itemsBag.size() + "/" + tran + ")");
                return true;
            }
            case 518:
                if (player.inventory.itemsBox.size() >= Inventory.MAX_ITEMS_BOX) {
                    Service.gI().sendThongBao(player, "Rương đồ đã đạt tối đa "
                            + Inventory.MAX_ITEMS_BOX + " ô.");
                    return false;
                }
                ...
                Service.gI().sendThongBaoOK(player, "Rương đồ của bạn đã được mở rộng thêm 1 ô ("
                        + player.inventory.itemsBox.size() + "/"
                        + Inventory.MAX_ITEMS_BOX + ")");
                return true;
            case 988:
                // Vật phẩm này KHÔNG THỂ có tác dụng: Inventory.getGoldLimit()
                // trả về hằng số LIMIT_GOLD = 2.000 tỉ cho MỌI người chơi,
                // không có giới hạn riêng từng người lưu ở đâu cả. Điều kiện
                // cũ `getGoldLimit() < 2_000_000_000_000L` là `2e12 < 2e12`,
                // luôn SAI — nên luôn rơi xuống nhánh "đã đạt tối đa".
                //
                // Trả false để KHÔNG tiêu mất vật phẩm. Muốn nó có tác dụng
                // thật thì phải thêm giới hạn vàng riêng cho từng người chơi
                // (một cột trong bảng player) — chưa làm vì đụng lược đồ dữ
                // liệu, cần người dùng đồng ý trước.
                Service.getInstance().sendThongBao(player,
                        "Giới hạn vàng đang là tối đa cho mọi người chơi, "
                        + "vật phẩm này chưa dùng được.");
                return false;
        }
