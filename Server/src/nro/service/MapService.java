...
public class MapService {
    ...
    public List<Zone> getMapCapsule(Player pl) {
        ...
        // Danh sách điểm đến đọc từ bảng capsule_map — sửa trên panel, không
        // phải sửa mã rồi biên dịch lại. Bảng rỗng thì DAO tự gieo lại đúng
        // danh sách gõ cứng cũ, nên hành vi mặc định không đổi.
        for (nro.repository.dao.CapsuleMapDAO.Diem d
                : nro.repository.dao.CapsuleMapDAO.danhSach()) {
            if (!d.bat || !hopDieuKien(pl, d)) {
                continue;
            }
    ...
    /** Người chơi có thoả điều kiện hiện của một điểm đến capsule không. */
    private boolean hopDieuKien(Player pl, nro.repository.dao.CapsuleMapDAO.Diem d) {
        if (d.dieuKien == null) {
            return true;
        }
        switch (d.dieuKien) {
            case nro.repository.dao.CapsuleMapDAO.LUYEN_TAP:
                return pl.levelLuyenTap > d.nguong;
            case nro.repository.dao.CapsuleMapDAO.SUC_MANH:
                return pl.nPoint != null && pl.nPoint.power > d.nguong;
            default:
                return true;
        }
    }
