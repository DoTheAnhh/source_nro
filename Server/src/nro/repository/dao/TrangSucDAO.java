...
public class TrangSucDAO {
     ...
     *
     * <h3>Mỗi mức là tỉ lệ của RIÊNG dòng đó</h3>
     *
     * <p>Dòng thứ nhất luôn có — bông tai không bao giờ rỗng chỉ số. Mức "2
     * dòng 50%" nghĩa là <b>dòng thứ hai</b> có 50% cơ hội xuất hiện, mức "3
     * dòng 20%" nghĩa là dòng thứ ba có 20% — không phải phần trăm của cả
     * bảng, nên tổng không cần bằng 100.</p>
     *
     * <p>Gieo <b>theo thứ tự</b> và dừng ngay ở mức đầu tiên trượt: có dòng ba
     * mà không có dòng hai là vô nghĩa, vì đây chỉ là đếm số dòng. Thiếu hẳn
     * mức nào trong bảng (khai 2 rồi nhảy sang 4) thì cũng dừng ở đó.</p>
     */
    public static int bocSoDong(int cap) {
        List<SoDong> ds = dsSoDong(cap, true);
        // dsSoDong tra ve theo so dong GIAM DAN; o day can tang dan.
        java.util.List<SoDong> tang = new ArrayList<>(ds);
        java.util.Collections.sort(tang, (a, b) -> a.soDong - b.soDong);
        int so = 1;
        for (SoDong x : tang) {
            if (x.soDong <= 1) {
                continue;
            ...
            if (x.tiLe <= 0 || !nro.core.util.Util.isTrue(x.tiLe, 100d)) {
                break;
            }
            so = x.soDong;
        }
    ...
    /**
     * Bảo đảm cấp nào cũng có mức "1 dòng · 100%".
     *
     * <p>Dòng thứ nhất là mặc định của hệ thống chứ không phải một lựa chọn, nên
     * nó hiện trên panel để người khai thấy đủ, và bị khoá không cho sửa hay
     * xoá. Không có dòng này thì bảng đọc như thể bông tai có thể ra <b>không</b>
     * dòng nào.</p>
     */
    public static void damBaoMucMotDong(int cap) {
        damBaoBang();
        for (SoDong x : dsSoDong(cap, false)) {
            if (x.soDong == 1) {
                if (x.tiLe != 100d || !x.bat) {
                    x.tiLe = 100d;
                    x.bat = true;
                    luuSoDong(x);
                }
    ...
    /** Xoá một mức. Mức 1 dòng không xoá được — nó là dòng luôn có. */
    public static String xoaMucSoDong(int cap, int id) {
        for (SoDong x : dsSoDong(cap, false)) {
            if (x.id == id && x.soDong <= 1) {
                return "Mức 1 dòng luôn 100%, không xoá được.";
            }
        }
        return xoaSoDong(id);
    }
