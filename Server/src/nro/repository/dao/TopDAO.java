 ...
 *
 * <p><b>Đây là nguồn tải CSDL nặng.</b> {@code ServerManager.autoUpdateBxh()} gọi
 * lớp này <b>mỗi 30 giây, bất kể có ai đang xem hay không</b> — 2.880 lượt mỗi ngày,
 * mỗi lượt là một loạt truy vấn có sắp xếp trên bảng {@code player}.</p>
 *
 * <p>Nên giãn chu kỳ, hoặc chỉ nạp lại khi thật sự có người mở bảng xếp hạng.</p>
 */
public class TopDAO {
     ...
     *
     * <p>Không lọc theo hành tinh: cả ba hành tinh xếp chung một bảng, tên hành
     * tinh hiện ngay trong từng dòng.</p>
     *
     * <p>{@code data_may_dam} là mảng JSON, ô {@code [0]} giữ kỷ lục này.</p>
     */
    public static final String TOP_DON_MANH_NHAT = "SELECT name, gender, items_body, data_may_dam, "
            + "CAST(JSON_UNQUOTE(JSON_EXTRACT(data_may_dam, '$[0]')) AS UNSIGNED) AS don_manh, LastTimeLoginGame "
            + "FROM player "
     ...
     * 30 giây, lấy lượt cao nhất từ trước tới nay. Ô {@code [1]} của mảng.
     */
    public static final String TOP_DAME_30S = "SELECT name, gender, items_body, data_may_dam, "
            + "CAST(JSON_UNQUOTE(JSON_EXTRACT(data_may_dam, '$[1]')) AS UNSIGNED) AS dame_30s, LastTimeLoginGame "
            + "FROM player "
            + "INNER JOIN account ON account.id = player.account_id "
            + "WHERE account.ban = 0 "
