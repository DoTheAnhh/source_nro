...
import nro.repository.dao.TopDAO;
import nro.entity.item.Item;
import nro.service.item.ItemService;
import nro.gameplay.top.TOP;
...
public class TopServer {
    ...
    public static List<TOP> TOP_DON_MANH_NHAT;
    public static List<TOP> TOP_DAME_30S;
    public static long timeRealTop = 0;
    
    /**
     * Doc mot o trang bi trong chuoi items_body da parse.
     *
     * <p>Ban cu goi thang {@code dataArray.get(i).toString()} nen chi can mot
     * tai khoan co items_body rong, null hay thieu o la NPE â ma NPE do nem ra
     * giua {@code loadDatabase} khien may chu khong mo cong 14445 luon.</p>
     *
     * @return mang o do, hoac null neu o khong doc duoc.
     */
    private static JSONArray oTrangBi(JSONArray dataArray, int viTri) {
        if (dataArray == null || viTri >= dataArray.size() || dataArray.get(viTri) == null) {
            return null;
        }
        Object o = JSONValue.parse(dataArray.get(viTri).toString());
        if (!(o instanceof JSONArray)) {
            return null;
        }
        JSONArray oDo = (JSONArray) o;
        return oDo.isEmpty() ? null : oDo;
    }
    /** Lần gần nhất nạp lại hai bảng máy đấm. */
    private static long lanNapMayDam = 0;

    /**
     * Nạp lại hai bảng máy đấm từ CSDL, nhiều nhất 15 giây một lần.
     *
     * <p>{@code Topserver_data} chỉ chạy đúng một lần lúc khởi động, nên mọi
     * bảng xếp hạng đứng im suốt phiên chạy — đấm gãy tay xong mở bảng vẫn thấy
     * 0. Hai bảng này thay đổi liên tục nên nạp lại ngay lúc người chơi mở, có
     * chặn nhịp để cả map cùng bấm cũng không nện CSDL.</p>
     */
    public static synchronized void napLaiTopMayDam() {
        if (System.currentTimeMillis() - lanNapMayDam < 15_000L) {
            return;
        }
        try (Connection con = ConnectDB.getConnection()) {
            TOP_DON_MANH_NHAT = realTop(TopDAO.TOP_DON_MANH_NHAT, con);
            TOP_DAME_30S = realTop(TopDAO.TOP_DAME_30S, con);
            lanNapMayDam = System.currentTimeMillis();
        } catch (Exception ex) {
            Logger.logException(TopServer.class, ex,
                    "Lỗi nạp lại bảng máy đấm");
        }
    }
    ...
    private static List<TOP> realTop(String query, Connection con) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                JSONArray dataItem = oTrangBi(dataArray, 0);
...
//                                top3Whis = rs.getLong("id");
                    ...
                    case TopDAO.TOP_DON_MANH_NHAT: {
                        Timestamp setLastTimeLogin = rs.getTimestamp("LastTimeLoginGame");
                        long firstTimeLoginLong = setLastTimeLogin != null ? setLastTimeLogin.getTime() : 0;
                        top.setLastTimeLogin(firstTimeLoginLong);
                        top.setDonManhNhat(rs.getLong("don_manh"));
                        i++;
                        break;
                    }
                    case TopDAO.TOP_DAME_30S: {
                        Timestamp setLastTimeLogin = rs.getTimestamp("LastTimeLoginGame");
                        long firstTimeLoginLong = setLastTimeLogin != null ? setLastTimeLogin.getTime() : 0;
                        top.setLastTimeLogin(firstTimeLoginLong);
                        top.setDame30s(rs.getLong("dame_30s"));
                        i++;
                        break;
                    }
                }
    ...
    public static void LoadingTop() {
        ...
        try (Connection con = ConnectDB.getConnection();) {
            ...
            Logger.logException(Manager.class, e, "Lỗi Loadding Top");
            System.exit(0);
        } finally {
            try {
                if (rs != null) {
