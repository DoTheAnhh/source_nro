...
public class ShopBot {
    ...
    public String getChat() {
        ...
        switch (this.idItem) {
            case 441:
                itemDesc = String.format(" %s %s", it.template.name, Util.isTrue(50, 100) ? "spl hút máu" : "spl hm");
                break;
            case 442:
                itemDesc = String.format(" %s %s", it.template.name, Util.isTrue(50, 100) ? "spl hút ki" : "spl hk");
                break;
            case 443:
                itemDesc = String.format(" %s %s", it.template.name, Util.isTrue(50, 100) ? "spl phản sát thương" : "spl pst");
                break;
            case 447:
                itemDesc = String.format(" %s %s", it.template.name, Util.isTrue(50, 100) ? "spl tnsm" : "spl smtn");
                break;
            default:
                itemDesc = String.format(" %s", it.template.name);
                break;
        }
        ...
        String quantityText;
        if (idItem == 380 || idItem == 934 || (idItem >= 220 && idItem <= 224) || (idItem >= 17 && idItem <= 20) || idItem == 2012) {
            quantityText = "/x99";
        } else if (idItem >= 441 && idItem <= 447) {
            quantityText = "/x3";
        } else {
            quantityText = Util.isTrue(50, 100) ? "/x1" : "/1";
        }
    ...
    public void chat(){
        switch (bot.gender) {
            case 0:
                if (Util.canDoWithTime(lastimeChattraidat, Util.nextInt(500000, 1000000))){
                    if (this.bot.zone != null) {
                ...
                break;
            case 1:
                if (Util.canDoWithTime(lastimeChatnamec, Util.nextInt(500000, 1000000))){
                    if (this.bot.zone != null) {
                        ChatGlobalService.gI().chatTGbot(this.bot , this.getChat());
                    }
                ...
                break;
            default:
                if (Util.canDoWithTime(lastimeChatxayda, Util.nextInt(500000, 1000000))){
                    if (this.bot.zone != null) {
                        ChatGlobalService.gI().chatTGbot(this.bot , this.getChat());
                    }
        ...
        if (Util.canDoWithTime(lastimeChatTrain, Util.nextInt(5000, 8000))){
            if (this.bot.zone != null) {
   ...
   public void activeTraDe(Player pl){
    ...
    public void active(int sl) {
        ...
        if (idItem == 380 || (idItem >= 220 && idItem <= 224) || (idItem >= 17 && idItem <= 20 || idItem == 2012)) {
            quantity *= 99;
        } else if (idItem >= 441 && idItem <= 447) {
            quantity *= 3;
        }
        ...
        // Thêm option nếu có
        if (idItem == 380 || (idItem >= 220 && idItem <= 224) || (idItem >= 1150 && idItem <= 1154)) {
            switch (idItem) {
                case 220:
                    optionId = 71;
                    break;
            ...
            it.addOptionParam(optionId, (optionId == 98 || optionId == 99 ? 3 : 5));
        } else {
            it.addOptionParam(optionId, 0);
        }
