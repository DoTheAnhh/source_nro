...
public class GiftCodeService {
    ...
    public void giftCode(Player player, String code) throws Exception {
        ...
        // Dung tham so hoa (PreparedStatement) - `code` la chuoi do nguoi choi go,
        // noi thang vao SQL se bi SQL injection.
        CrisResultSet rs = ConnectDB.executeQuery(
                "SELECT * FROM giftcode_save WHERE `player_id` = ? AND `code_da_nhap` = ?;",
                player.id, code);
        if (rs != null && rs.first()) {
