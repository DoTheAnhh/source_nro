...
public class TopService {
    ...
    /** Tên hành tinh để ghi vào từng dòng bảng xếp hạng. */
    private static String tenHanhTinh(byte gender) {
        switch (gender) {
            case 0:
                return "Trái Đất";
            case 1:
    ...
    /**
     * Bảng "Đòn Mạnh Nhất" — kỷ lục một đòn đơn vào máy đo sức mạnh.
     *
     * <p>Ba hành tinh chung một bảng, tên hành tinh ghi ngay trong dòng.</p>
     */
    public static void showTopDonManhNhat(Player player) {
        TopServer.napLaiTopMayDam();
        List<TOP> tops = TopServer.TOP_DON_MANH_NHAT;
        Message msg = null;
        try {
            msg = new Message(-96);
            msg.writer().writeByte(0);
            msg.writer().writeUTF("Top Đòn Mạnh Nhất");
            ...
            for (int i = 0; i < tops.size(); i++) {
                ...
                // Chuỗi này hiện thẳng trong ô của bảng — nhồi cả điểm lẫn hành
                // tinh vào đây vì client chỉ vẽ đúng một dòng cho mỗi mục.
                msg.writer().writeUTF(Util.format(top.getDonManhNhat())
                        + " sát thương - " + tenHanhTinh(top.getGender()));
                // Chuỗi này chỉ hiện khi bấm vào dòng, nên viết dài được.
                msg.writer().writeUTF("Đòn mạnh nhất: "
                        + Util.formatNumber(top.getDonManhNhat(), FormatStyle.VIETNAMESE)
                        + " sát thương trong một cú đấm."
                        + "\nHành tinh: " + tenHanhTinh(top.getGender())
                        + "\nĐăng nhập " + getTimeLeft(top.getLastTimeLogin()));
            }
    ...
    /**
     * Bảng "Sát Thương 30 Giây" — tổng sát thương gom được trong một lượt đấm
     * dài 30 giây, lấy lượt cao nhất từ trước tới nay.
     */
    public static void showTopDame30s(Player player) {
        TopServer.napLaiTopMayDam();
        List<TOP> tops = TopServer.TOP_DAME_30S;
        Message msg = null;
        try {
            msg = new Message(-96);
            msg.writer().writeByte(0);
            msg.writer().writeUTF("Top Sát Thương Trong 30 Giây");
            msg.writer().writeByte(tops.size());
            for (int i = 0; i < tops.size(); i++) {
                TOP top = tops.get(i);
                msg.writer().writeInt(i + 1);
                ...
                msg.writer().writeShort(top.getLeg());
                msg.writer().writeUTF(top.getName());
                msg.writer().writeUTF(Util.format(top.getDame30s())
                        + " sát thương - " + tenHanhTinh(top.getGender()));
                msg.writer().writeUTF("Tổng sát thương đấm được trong 30 giây: "
                        + Util.formatNumber(top.getDame30s(), FormatStyle.VIETNAMESE)
                        + "\nHành tinh: " + tenHanhTinh(top.getGender())
                        + "\nĐăng nhập " + getTimeLeft(top.getLastTimeLogin()));
            }
            player.sendMessage(msg);
