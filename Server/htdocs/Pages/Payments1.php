...
$userId   = $ImS['id'];
// Lấy cấu hình website (bank, qr, ...)
$settingQuery = $Connect->query("SELECT * FROM settings LIMIT 1");
$Settings = $settingQuery->fetch(PDO::FETCH_ASSOC);

if (!$Settings) {
    die('Chưa có dữ liệu bảng settings');
}
...
$query2 = "SELECT * FROM payments WHERE name = '$userId' ORDER BY id DESC LIMIT $start2, $limit2";
$result2 = $Connect->query($query2);
$totalQuery2 = "SELECT COUNT(*) as total FROM payments WHERE name = '$userId'";
$totalResult2 = $Connect->query($totalQuery2);
$totalRows2 = $totalResult2->fetch(PDO::FETCH_ASSOC)['total'];
        ...
        <div class="box_list_chuyenmuc">
            <div class="box_midss">
                <div class="box_detai" style="padding:5px;">
                    ...
                    <!-- Hiển thị QR và thông tin chuyển khoản theo 2 cột -->
                    <div style="max-width:500px; margin: 0 auto; display:flex; align-items:center; border:1px solid #ccc; border-radius:8px; overflow:hidden; box-shadow: 0px 4px 10px rgba(0,0,0,0.1);">
                        <div style="flex:0 0 40%; background-color: #f5f5f5; text-align:center; padding:10px;">
                            <img src="https://img.vietqr.io/image/MB-<?= $Settings['NumberBank'] ?>-qr_only.png
                            ?amount=<?= isset($a) ? $a : 0 ?>
                            &addInfo=<?= urlencode('naptien ' . $ImS['id']) ?>
                            &accountName=<?= urlencode($Settings['NameBank']) ?>"
                            alt="QR Code"
                            style="max-width:100%; height:auto;">
                        </div>
                        <div style="flex:1; padding:10px; font-family: 'Times New Roman', serif;">
                            <p style="margin:5px 0;"><b>Tên Tài Khoản:</b> <?= $Settings['NameBank'] ?></p>
                            <p style="margin:5px 0;"><b>Số Tài Khoản:</b> <?= $Settings['NumberBank'] ?></p>
                            <p style="margin:5px 0;"><b>Ngân Hàng:</b> Quân Đội MBBank</p>
                            <p style="margin:5px 0;"><b>Nội Dung:</b> naptien <?= $ImS['id'] ?></p>
                        </div>
                    </div>
                    <br>
                    <hr>
                    ...
                    <div id="pagination2" style="text-align:center; padding:15px 0;">
                        <?php for ($i = 1; $i <= $totalPages2; $i++) { ?>
                            <a href="?page2=<?= $i; ?>"
                                style="display: inline-block; padding: 6px 12px; margin-right: 4px; text-decoration: none; border: 1px solid #2a9d8f; border-radius: 4px; transition: 0.3s; 
