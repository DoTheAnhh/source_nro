...
public class GodGK {
                    ...
                    if (secondsPass < Manager.SECOND_WAIT_LOGIN) {
                        ...
                        if (!rs.first()) {
                            ...
                            if ((player = loadPlayer(rs, false)) != null) {
                                ...
                                player.isNewMember = !Util.isTimeDifferenceGreaterThanNDays(createTime, session.accountAgeDays);
                                player.accountCreatedDays = Util.getDaysSince(createTime);
                                ConnectDB.executeUpdate("update account set last_time_login = '" + new Timestamp(System.currentTimeMillis()) + "', ip_address = '" + session.ipAddress + "' where id = " + session.userId);
                            }
                        }
                    }
                }
    ...
    private static Player loadPlayer(CrisResultSet rs, boolean isOffline) throws Exception {
        ...
        try {
            ...
            if (player.CHECK_DAY_ON_ONE == 0) player.CHECK_DAY_ON_ONE = (ngayhomnay - 1);
            ...
            if (player.CHECK_DAY_ON_TUAN == 0) player.CHECK_DAY_ON_TUAN = (ngayhomnay2 - 1);
            ...
            if (player.CHECK_DAY_ON_THANG == 0) player.CHECK_DAY_ON_THANG = (ngayhomnay3 - 1);
            ...
            if (player.CHECK_DAY_ON_NAM == 0) player.CHECK_DAY_ON_NAM = (ngayhomnay4 - 1);
            ...
            if (player.CHECK_DAY_ON_CHI_TON == 0) player.CHECK_DAY_ON_CHI_TON = (ngayhomnay5 - 1);
            ...
            Calendar ngayvps = Calendar.getInstance();
            int ngayhomnay = ngayvps.get(Calendar.DAY_OF_YEAR);
            if (player.TimeTrongNgay == 0) player.TimeTrongNgay =(int)(ngayhomnay-1);
            ...
            Calendar ngayvps = Calendar.getInstance();
            int ngayhomnay = ngayvps.get(Calendar.DAY_OF_YEAR);
            if (player.CheckTrongNgay == 0) player.CheckTrongNgay =(int)(ngayhomnay-1);
