...
namespace Game4
{
    ...
    public class mFont
    {
    	...
    	// ------------------------------------------------------------------
    	//  Đồ dùng lại — đừng cấp phát gì trong lúc vẽ
    	// ------------------------------------------------------------------
    
    	/// <summary>
    	/// Kiểu chữ dùng chung cho mọi lần vẽ của phông này.
    	/// </summary>
    	/// <remarks>
    	/// <para>Bản trước dựng <c>new GUIStyle(GUI.skin.label)</c> <b>mỗi lần vẽ
    	/// một chuỗi</b>. Đó là một đối tượng có phần thân bên máy, chép lại cả bộ
    	/// lề, phông, cỡ chữ — đắt hơn hẳn một cấp phát thường.</para>
    	///
    	/// <para>Chữ có viền còn nhân lên nữa: <c>setTypePaint</c> vẽ chín lần cho
    	/// tám hướng viền, mà <c>drawStringBd</c> lại gọi <c>setTypePaint</c> chín
    	/// lần — tám mươi mốt cái <c>GUIStyle</c> cho <i>một</i> dòng chữ. Một khung
    	/// hình HUD có vài chục dòng như thế.</para>
    	///
    	/// <para>Giữ một cái rồi đổi màu với căn lề trước mỗi lần vẽ: <c>GUI.Label</c>
    	/// vẽ ngay trong lời gọi nên không ai giữ lại cái kiểu chữ này sau đó.</para>
    	/// </remarks>
    	private GUIStyle kieuChu;
    
    	/// <summary>Kiểu chữ chỉ dùng để đo bề rộng, chung cho mọi phông.</summary>
    	private static GUIStyle kieuDo;
    
    	/// <summary>Hộp chữ dùng lại khi đo — <c>new GUIContent</c> mỗi lần là rác.</summary>
    	private static GUIContent hopDo;
    
    	/// <summary>Bề rộng đã đo, nhớ lại theo chuỗi.</summary>
    	/// <remarks>
    	/// <c>CalcSize</c> chạy cả bộ dàn chữ. Chuỗi trên HUD thì lặp lại y hệt từ
    	/// khung này sang khung khác — tên nhân vật, "0/12", nhãn nút — nên đo một
    	/// lần là đủ.
    	/// </remarks>
    	private System.Collections.Generic.Dictionary<string, int> khoBeRong;
    
    	/// <summary>Mức phóng lúc dựng <see cref="khoBeRong"/>.</summary>
    	private int zoomCuaKho = -1;
    
    	private static Color mauTu(int rgb)
    	{
    		return new Color((float)((rgb >> 16) & 0xFF) / 256f,
    				(float)((rgb >> 8) & 0xFF) / 256f,
    				(float)(rgb & 0xFF) / 256f);
    	}
    
    	/// <summary>Sáu màu viền của phông cỡ lớn — dựng một lần.</summary>
    	private static readonly Color[] MAU_VIEN_LON = {
    		mauTu(6029312), mauTu(7169025), mauTu(7680),
    		mauTu(0), mauTu(9264), mauTu(6029312)
    	};
    
    	/// <summary>Bảy màu thân của phông cỡ lớn — dựng một lần.</summary>
    	private static readonly Color[] MAU_THAN_LON = {
    		Color.red, Color.yellow, Color.green, Color.white,
    		mauTu(40404), Color.red, Color.black
    	};
    	...
    	// Mac dinh KHONG duoc la trang: day la nguon duy nhat sinh ra chu mau trang
    	// trong ca he thong font (moi mau cau hinh trong colorJava va moi file PNG
    	// deu la mau toi). Lan ve nao roi vao trang thai chua gan mau se lay gia tri
    	// nay, va truoc day no la Color.white -> chu bien thanh trang.
    	public Color color1 = new Color32(0x53, 0x29, 0x05, 0xFF);
    	...
    	public void drawString(mGraphics g, string st, int x, int y, int align)
    	{
    		if (mGraphics.zoomLevel == 1)
    		{
    			...
    			{
    				// Tim theo KY TU, khong ghep chuoi: `st[i] + string.Empty` la
    				// mot chuoi moi cho MOI ky tu cua MOI dong chu ve ra.
    	...
    	public int getWidthExactOf(string s)
    	{
    		try
    		{
    			...
    			if (khoBeRong == null || zoomCuaKho != mGraphics.zoomLevel)
    			{
    				khoBeRong = new System.Collections.Generic.Dictionary<string, int>();
    				zoomCuaKho = mGraphics.zoomLevel;
    			}
    			int daBiet;
    			if (khoBeRong.TryGetValue(s, out daBiet))
    			{
    				return daBiet;
    			}
    			if (kieuDo == null)
    			...
    			if (hopDo == null)
    			{
    				hopDo = new GUIContent();
    			}
    			kieuDo.font = myFont;
    			hopDo.text = s;
    			int be = (int)kieuDo.CalcSize(hopDo).x / mGraphics.zoomLevel;
    			if (khoBeRong.Count > 2048)
    			{
    				khoBeRong.Clear();
    			}
    			khoBeRong[s] = be;
    	...
    	public int getHeight()
    	{
    		...
    		try
    		{
    			height = (int)kieuDo.CalcSize(hopDo).y + 2;
    		}
    		catch (Exception ex)
    		{
    	...
    	public void _drawString(mGraphics g, string st, int x0, int y0, int align)
    	{
    		...
    		if (kieuChu == null)
    		{
    			kieuChu = new GUIStyle(GUI.skin.label);
    		}
    		GUIStyle gUIStyle = kieuChu;
    		...
    		// Phai gan MOI trang thai, khong chi normal. GUI.Label cua IMGUI tu doi sang
    		// trang thai hover khi con tro nam trong Rect, va lay mau tu hover.textColor
    		// - gia tri nay thua ke tu GUI.skin.label nen la mau TRANG. Do la ly do chu
    		// bien thanh trang khi re chuot vao, du khong dong code nao cua game chon
    		// mau trang. Rect o day rong ca man hinh nen rat de trung.
