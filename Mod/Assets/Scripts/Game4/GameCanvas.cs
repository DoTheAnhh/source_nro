...
namespace Game4
{
    ...
    public class GameCanvas : IActionListener
    {
    	...
    	/// <summary>
    	/// Cú bấm hướng vừa rồi đến từ W A S D hay từ bốn phím mũi tên.
    	/// </summary>
    	/// <remarks>
    	/// <para>Thứ tự: 0 = W, 1 = S, 2 = A, 3 = D.</para>
    	///
    	/// <para>W A S D và bốn mũi tên cùng đẩy xuống một mã (-1…-4), nên tới nơi
    	/// xử lý thì không còn phân biệt được. Cờ này giữ lại chỗ xuất phát, để bản
    	/// máy tính cho W A S D <b>dịch chuyển mà không phá trạng thái</b> — đang
    	/// độn thổ thì độn thổ mà đi, đang khinh công thì lơ lửng mà đi.</para>
    	/// </remarks>
    	public static bool[] phimWasd = new bool[4];
    	...
    	public void initGame()
    	{
    		...
    		// Chua ai luu thi loadRMSInt tra -1, ma dieu kien cu la "== 1" nen ra
    		// FALSE — hao quang cua CHINH MINH khong bao gio duoc ve (dieu kien
    		// ve la "!me || isPaintAura"). Nay chi tat khi da luu ro rang so 0.
    		Char.isPaintAura = Rms.loadRMSInt("isPaintAura") != 0;
    		// Cung ly do: chua luu thi -1, phai coi la BAT.
    	...
    	public void update()
    	{
    		...
    		try
    		{
    			...
    			// Khung chat duoc hoi cham TRUOC bang, menu va hop thoai.
    			//
    			// No nam tren cung khi ve, nen no cung phai duoc quyen an cham
    			// truoc — khong thi bam vao the chat ma bang phia duoi lai nhan.
    			if (God.ChatUI.getInstance().capNhatCham())
    			{
    				return;
    			}
                    if (!TabControll.Instance.isPointerHoldInTab()  && currentScreen != null)
    			{
    				// Menu chon nhan cham TRUOC ca chuoi duoi day.
    				//
    				// Chuoi else-if chi cho MOT nhanh chay moi khung; menu nam
    				// giua chuoi nen bat ky popup hay hop thoai nao con sot cung
    				// cuop mat luot cua no. Tach ra ngoai thi menu luon duoc hoi,
    				// khong phu thuoc thu tu.
    				if (menu.showMenu)
    				{
    					menu.updateMenu();
    					menu.updateMenuKey();
    				}
    				// Menu duoc xet TRUOC popup va hop thoai.
    				//
    				// `Menu.startAt` dat menu o duoi day man hinh (menuTemY =
    				// GameCanvas.h) roi doi `updateMenu()` keo len. Trong chuoi
    				// else-if cu, nhanh cua menu chi chay khi KHONG con popup nao
    				// — nen chi can mot popup ket lai la menu nam im duoi day,
    				// vo hinh, va moi menu sau do cung the. Dung hien tuong "lan
    				// dau bam thi hien, tu lan hai tro di khong hien gi ca", ke ca
    				// nut "cat vao hanh trang" cua tung mon.
    				//
    				// Dat menu len truoc la dung y do san co: `startAt` da tu xoa
    				// `currChatPopup` khi mo menu, tuc thiet ke von coi menu la
    				// lop tren. Popup van duoc VE binh thuong, chi khong gianh
    				// mat luot cap nhat cua menu nua.
    				if (menu.showMenu)
    				{
    					// Menu da duoc cap nhat o khoi ngay tren, KHONG lam lai.
    					//
    					// Goi hai lan trong mot khung hinh thi bo dem khungTuLucMo
    					// nhay hai buoc moi khung, nen cai chot "bo qua ba khung dau
    					// sau khi menu mo" chi con che duoc mot khung ruoi. Cu bam
    					// MO bang NPC vi the con kip ngo sang chinh bang vua hien
    					// va tu chon muc dau — mot cai Enter an hai lan.
    					//
    					// Van giu nhanh nay de chuoi else-if ben duoi dung lai o
    	...
    	/// <summary>
    	/// Tinh lai be rong/cao khung ao va cac gia tri dan xuat.
    	/// </summary>
    	/// <remarks>
    	/// <para>Tach ra khoi <c>initGameCanvas</c> de goi lai duoc GIUA luc dang
    	/// choi, khi nguoi choi doi qua lai giua cua so va toan man hinh.</para>
    	///
    	/// <para>Khong goi ca <c>initGameCanvas</c> vi ham do con tao lai
    	/// <c>inputDlg</c> va <c>listPoint</c> — goi lai giua game la xoa mat hop
    	/// nhap dang mo va danh sach diem dang giu.</para>
    	///
    	/// <para>Can tinh lai vi <c>GameCanvas.w = Screen.width / zoomLevel</c>:
    	/// cua so 1024x600 cho khung ao 512x300, con toan man hinh 1080p cho
    	/// 960x540 — chenh gan gap doi. Khong tinh lai thi ca HUD van xep theo co
    	/// cu, tuc la don het ve mot goc man hinh.</para>
    	/// </remarks>
    	public void capNhatKichThuoc()
    	{
    		w = MotherCanvas.instance.getWidthz();
    		h = MotherCanvas.instance.getHeightz();
    		debug("SP2i2", 0);
    	...
    	public void paint(mGraphics gx)
    	{
    		try
    		{
    			...
    			InfoDlg.paint(g);
    			// Khung chat NAM TREN moi bang va menu.
    			//
    			// Truoc day no ve trong chuoi cua ClientManager, ma chuoi do chay
    			// ben trong GameScr.paint — tuc truoc bang, menu, hop thoai — nen mo
    			// bat ky bang nao la khung chat bi de len va mat hut.
    			//
    			// Ve o day thi no o tren; hai hop nhap ngay duoi lai o tren no, dung
    			// nhu can: dang go chu thi cai o dang go phai la thu tren cung.
    			God.ChatUI.getInstance().ve(g);
    			God.BanPhimSo.getInstance().ve(g);
    			...
    			// Ve menu chon o DAY, khong dua vao cho ve cu o giua ham.
    			//
    			// Cho cu nam trong mot nhanh else-if sau `currentDialog`, va do
    			// bang log thi no CO chay, toa do CO nam trong man hinh, nhung
    			// khong ra hinh. `TabControll.Instance.paint` ngay tren thi luon
    			// ra hinh — nut Tab hien trong moi anh chup — nen ve canh no la
    			// chac chan thay.
    			if (menu.showMenu)
    			{
    				resetTrans(g);
    	...
    	public void perform(int idAction, object p)
    	{
    		...
    		case 88836:
    			...
    			// Tat go tieng Viet: day la mat khau so.
    			God.HopNhapChu.getInstance().moRa(mResources.INPUT_PRIVATE_PASS,
    					string.Empty, 6, false, chu =>
    					{
    						if (chu.Length < 6)
    						{
    						...
    						try
    						{
    							Service.gI().activeAccProtect(int.Parse(chu));
    						}
    						catch (Exception loi)
    						{
    							startOKDlg(mResources.ALERT_PRIVATE_PASS_2);
    							Cout.println("Loi mat khau bao ve: " + loi.ToString());
    						}
    					});
    			break;
    		case 888361:
