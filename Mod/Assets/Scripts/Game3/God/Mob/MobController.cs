...
namespace Game3.God
{
    public class MobController
    {
        ...
        public static void Update()
        {
            ...
            // Đang mở menu mod, bảng, hay khung chat thì tạm dừng.
            //
            // Không phải để "lịch sự": mỗi lần đổi kỹ năng, GameScr gọi
            // resetButton(), mà hàm đó có GameCanvas.menu.showMenu = false. Tàn
            // Sát đổi kỹ năng liên tục nên bấm X mở menu mod là nó đóng ngay lập
            // tức — nhìn như menu không mở được.
            ...
            {
                // "Tàn Sát Người": chỉ đánh BOSS và người chơi, bỏ qua quái
                // thường. Người chơi được ưu tiên; hết người mới quay sang boss.
                if (!Mobs.tsPlayer)
                {
                    return;
                }
        ...
        /// <summary>Nhắm và đánh boss gần nhất — phần boss của "Tàn Sát Người".</summary>
        /// <remarks>
        /// Tách riêng chứ không dùng lại nhánh Tàn Sát thường: nhánh kia nhắm mọi
        /// quái, mà chế độ này chỉ được đụng vào boss và người chơi.
        /// </remarks>
        private static void DanhBoss(Char @char)
        {
            if (@char.mobFocus != null && !IsBossTanSat(@char.mobFocus))
            {
                @char.mobFocus = null;
            }
            if (@char.mobFocus == null)
            {
                @char.mobFocus = GetBossTanSat();
            }
            Mob boss = @char.mobFocus;
            if (boss == null || @char.skillInfoPaint() != null)
            {
                return;
            }
            Skill skill = GetSkillAttack2();
            ...
            if (Res.distance(boss.xFirst, boss.yFirst, @char.cx, @char.cy) <= 48)
            {
                ...
                else if (skill.template.isUseAlone())
                {
                    GameScr.gI().doDoubleClickToObj(boss);
                }
                return;
            }
            if (Mobs.telePem)
            {
                if (Math.abs(boss.xFirst - @char.cx) >= 20 || Math.abs(boss.yFirst - @char.cy) >= 20)
                {
                    MoveTo(boss.xFirst, boss.yFirst);
                }
                return;
            }
            Move(boss.xFirst, boss.yFirst);
        }

        /// <summary>Con này có phải boss đang sống và đánh được không.</summary>
        private static bool IsBossTanSat(Mob mob)
        {
            if (mob == null || mob.status == 0 || mob.status == 1
                    || mob.hp <= 0 || mob.isMobMe)
            {
                return false;
            }
            return mob.levelBoss > 0;
        }

        /// <summary>Boss gần nhất trong khu, hoặc null nếu không có con nào.</summary>
        private static Mob GetBossTanSat()
        {
            Mob result = null;
            int gan = int.MaxValue;
            Char @char = Char.myCharz();
            for (int i = 0; i < GameScr.vMob.size(); i++)
            {
                Mob mob = (Mob)GameScr.vMob.elementAt(i);
                int kc = (mob.xFirst - @char.cx) * (mob.xFirst - @char.cx)
                        + (mob.yFirst - @char.cy) * (mob.yFirst - @char.cy);
                if (IsBossTanSat(mob) && kc < gan)
                {
                    result = mob;
                    gan = kc;
                }
