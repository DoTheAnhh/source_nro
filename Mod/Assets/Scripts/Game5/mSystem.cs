...
namespace Game5
{
    ...
    public class mSystem
    {
    	...
    	/// <summary>Số tick của mốc 1970 — tính một lần.</summary>
    	private static readonly long TICK_1970 =
    			new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc).Ticks;

    	/// <summary>Mốc Unix (mili giây) lúc khởi động.</summary>
    	private static readonly long GOC_UNIX =
    			(DateTime.UtcNow.Ticks - TICK_1970) / 10000;

    	/// <summary>Đồng hồ bấm giây, chạy từ lúc khởi động.</summary>
    	private static readonly System.Diagnostics.Stopwatch DONG_HO =
    			System.Diagnostics.Stopwatch.StartNew();

    	/// <summary>
    	/// Thời gian hiện tại tính bằng mili giây kể từ mốc 1970.
    	/// </summary>
    	/// <remarks>
    	/// <para>Bản trước <b>dựng một <c>DateTime</c> mới ở mỗi lần gọi</b> chỉ để
    	/// lấy số tick của mốc 1970 — mà con số ấy là hằng số. Hàm này bị gọi khắp
    	/// nơi: riêng <c>Char.cs</c> đã hai mươi lăm chỗ, nhân với số nhân vật trên
    	/// bản đồ là vài trăm lần mỗi khung hình.</para>
    	///
    	/// <para>Nay mốc 1970 tính một lần, và phần trôi đi đọc từ
    	/// <c>Stopwatch</c> — bộ đếm nhịp của máy, rẻ hơn <c>DateTime.UtcNow</c>
    	/// nhiều. Kèm một cái lợi nữa: đồng hồ này <b>đơn điệu</b>, nên người chơi
    	/// đổi giờ hệ thống giữa phiên cũng không làm mấy bộ đếm lùi hay nhảy vọt.</para>
    	/// </remarks>
    	public static long currentTimeMillis()
    	{
    		return GOC_UNIX + DONG_HO.ElapsedMilliseconds;
    	}
    	...
    	public static FrameImage getFraImage(string nameImg)
    	{
    		...
    		{
    			// Chan chia cho 0: may chu tra nFrame = 0 khi ten anh chua co trong
    			// bang img_by_name (vua them dong ma chua khoi dong lai). Chia cho 0
    			// nem ngoai le GIUA luc ve, ca khung hinh con lai mat trang.
    			int soKhung = (mainImage.nFrame > 0) ? mainImage.nFrame : 1;
    			int num = mainImage.img.getHeight() / soKhung;
    			if (num < 1)
    			{
    				num = 1;
    			}
