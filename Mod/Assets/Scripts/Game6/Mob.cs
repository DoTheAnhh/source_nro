...
namespace Game6
{
    ...
    public class Mob : IMapObject
    {
    		...
    		if (!GameCanvas.lowGraphic && status != 1 && status != 0 && !GameCanvas.lowGraphic && GameCanvas.gameTick % (15 + mobId * 2) == 0)
    		{
    			for (int i = 0; i < GameScr.vCharInMap.size(); i++)
    			{
    				...
    				if (@char != null && @char.isFlyAndCharge && @char.cf == 32)
    				{
    					...
    					if (@char.cgender == 0 && !God.TrangPhucUI.laQCKK(@char))
    					{
    			...
    			if (Char.myCharz().isFlyAndCharge && Char.myCharz().cf == 32)
    			{
    				...
    				if (Char.myCharz().cgender == 0 && !God.TrangPhucUI.laQCKK(Char.myCharz()))
    				{
    	...
    	public virtual void paint(mGraphics g)
    	{
    		...
    		// num2 la be rong cua vet mau vua mat, tinh theo imageWidth cua bo anh
    		// cu. Doi nguoc ve PHAN TRAM de ham chung khong phai biet gi ve bo anh
    		// do — bo anh do khong con duoc ve nua.
    		int perVet = (imageWidth > 0) ? (num2 * 100 / imageWidth) : per;
    		GameScr.veThanhMauNoi(g, x, y - h - 6, GameScr.RONG_THANH_NOI,
    				GameScr.CAO_THANH_NOI, per, perVet);
    	}
